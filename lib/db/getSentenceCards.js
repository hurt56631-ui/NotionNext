    ...
    try {
        ...
        for (let i = 0; i < pageIds.length; i++) {
          ...
          if (isPublished) {
            ...
            // 【核心修正】: 正确解析 Relation 属性
            // Notion 的 Relation 属性返回的是一个复杂的数组结构，我们需要从中提取出关联页面的 ID
            const relatedCourseIds = properties['所属课程']?.[0]?.[1]?.map(relation => relation[1]) || [];
            ...
            const cleanTopic = { 
                ...
                courseIds: relatedCourseIds // 使用我们刚刚正确解析出的 ID 数组
            };
            data.push(cleanTopic);
            // 增强日志，确认关联ID是否被正确解析
            if(i < 5) { // 只打印前5条日志，避免刷屏
