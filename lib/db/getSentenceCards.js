...
export async function getSentenceCards({ databaseId }) {
    if (!databaseId) {
        console.warn('【日志-源头-卡片】警告: 未提供句子卡片的 Database ID。');
        return [];
    }
    try {
        console.log('【日志-源头-卡片】开始获取句子卡片, Database ID:', databaseId);
        const pageRecordMap = await getPage(databaseId, 'card-db');
        if (!pageRecordMap) {
            console.error('【日志-源头-卡片】错误: 无法获取卡片数据库的 pageRecordMap，请检查ID和权限。');
            return [];
        }
        ...
        if (rawMetadata?.type !== 'collection_view_page' && rawMetadata?.type !== 'collection_view') {
            console.error(`【日志-源头-卡片】错误: ID "${id}" 不是一个有效的 Notion 数据库。`);
            return [];
        }
        ...
        console.log(`【日志-源头-卡片】在卡片库中找到了 ${pageIds.length} 个页面。`);
        ...
        console.log(`【日志-源头-卡片】成功处理并返回了 ${data.length} 张已发布的卡片。`);
        return data;
    } catch (error) { 
        console.error('【日志-源头-卡片】获取句子卡片时出错:', error); 
        return []; 
    }
}
