...
export async function getSentenceCards({ databaseId }) {
    ...
    try {
        ...
        const pageRecordMap = await getPage(databaseId, 'card-db-final-perfect');
        if (!pageRecordMap) {
        ...
        for (const pageId of pageIds) {
          ...
          if (isPublished) {
            ...
            // ====================== 【最终核心修正】 ======================
            // 不再相信 properties['所属课程']，我们手动从原始数据中解析
            // 1. 在 schema 中找到 '所属课程' 对应的内部 key (例如 'ZtIq')
            const relationSchemaKey = Object.keys(schema).find(key => schema[key].name === '所属课程');
            let manualCourseIds = [];

            // 2. 如果找到了 key，并且这个卡片页面确实有关联数据
            if (relationSchemaKey && value.properties && value.properties[relationSchemaKey]) {
                // 3. Notion 的关联数据结构是 [['‣', [['p', '关联页面ID']]]]
                // 我们深入这个结构，提取出所有 '关联页面ID'
                const relations = value.properties[relationSchemaKey][0][1];
                if (relations) {
                    manualCourseIds = relations.map(r => r[1]); // r 的结构是 ['p', '页面ID']
                }
            }
            // =============================================================
            ...
            const cleanTopic = { 
                ...
                courseIds: manualCourseIds // <-- 使用我们手动解析出的、绝对正确的 ID 数组
            };
            data.push(cleanTopic);
            
            console.log(`  -> 【日志】成功处理卡片: "${cleanTopic.word}", 手动解析出的关联课程ID: [${cleanTopic.courseIds.join(', ')}]`);
