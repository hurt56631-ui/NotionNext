...
export async function getSpeakingCourses({ databaseId }) {
    ...
    try {
        ...
        if (!pageRecordMap) {
            console.error('【日志-源头-课程】错误: 无法获取课程数据库的 pageRecordMap。');
            return [];
        }
        ...
        for (let i = 0; i < pageIds.length; i++) {
            ...
            if (isPublished) {
                ...
                data.push(course);
                console.log(`  -> 成功处理课程: "${course.title}"`);
            }
        }
        console.log(`【日志-源头-课程】成功处理并返回了 ${data.length} 个课程。`);
        return data;
    } catch (error) { console.error('【日志-源头-课程】获取口语课程时发生严重错误:', error); return []; }
}
