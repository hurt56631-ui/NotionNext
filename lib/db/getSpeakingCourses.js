...
export async function getSpeakingCourses({ databaseId }) {
    if (!databaseId) {
        console.warn('【日志-源头-课程】警告: 未提供口语课程的 Database ID。');
        return [];
    }
    try {
        console.log('【日志-源头-课程】开始获取口语课程, Database ID:', databaseId);
        const pageRecordMap = await getPage(databaseId, 'course-db');
        if (!pageRecordMap) {
            console.error('【日志-源头-课程】错误: 无法获取课程数据库的 pageRecordMap，请检查ID和权限。');
            return [];
        }
        ...
        if (rawMetadata?.type !== 'collection_view_page' && rawMetadata?.type !== 'collection_view') {
            console.error(`【日志-源头-课程】错误: ID "${id}" 不是一个有效的 Notion 数据库。`);
            return [];
        }
        ...
        console.log(`【日志-源头-课程】在课程库中找到了 ${pageIds.length} 个页面。`);
        ...
        console.log(`【日志-源头-课程】成功处理并返回了 ${data.length} 个已发布的课程。`);
        return data;
    } catch (error) { 
        console.error('【日志-源头-课程】获取口语课程时发生严重错误:', error); 
        return []; 
    }
}
