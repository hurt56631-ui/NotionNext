...
export const AuthProvider = ({ children }) => {
  ...
  // --- 2. RTDB 在线状态维护逻辑 (依赖于 user 和 rtDb) ---
  useEffect(() => {
    // 检查 rtDb 是否已初始化 (避免在 firebase.js 导出 null 的情况下执行)
        ...
        const unsubscribeConnected = onValue(connectedRef, (snapshot) => {
            if (snapshot.val() === true) {
                onDisconnect(userStatusRef).set({ state: 'offline', last_changed: rtServerTimestamp() }).then(() => {
                    ...
                    // 辅助：每隔 60 秒更新一次 Firestore lastSeen 
  ...
  }, [user, rtDb]); // 依赖中必须包含 rtDb

  // --- 3. 全局未读数初始同步 + 实时监听 (依赖于 user) ---
  useEffect(() => {
    if (!user) {
