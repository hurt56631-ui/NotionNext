...
1.  **彻底解决了崩溃问题**：移除了 `getDocs` 和 `onSnapshot` 的冲突，现在只用一个稳定、高效的 `onSnapshot` 来处理全局未读数，从根源上消除了导致崩溃的渲染冲突。
2.  **修复了加载逻辑**：确保了只有在用户资料（无论是已存在还是新创建）被加载后，`authLoading` 状态才会变为 `false`，这可以防止页面因为数据未准备好而出现闪烁或错误。
3.  **简化了代码**：通过移除多余的查询和不必要的导入，代码现在更清晰、更易于维护。
4.  **保留了所有核心功能**：包括本地缓存用户数据、实时更新用户资料、实时维护在线状态、以及高效计算全局未读数。

请将这份代码部署后，你的应用应该会变得非常稳定，并且 Firestore 的读取成本也会保持在一个极低的水平。
