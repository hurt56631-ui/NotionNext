// themes/heo/index.js  <-- 最終調試版 v8：內置診斷工具
...
// ... 其他組件定義保持不變 ...
const LayoutBase = props => { const { children, slotTop, className } = props; const { fullWidth, isDarkMode } = useGlobal(); const router = useRouter(); if (router.route === '/') { return <>{children}</> } const headerSlot = ( <header> <Header {...props} /> {router.route === '/' ? (<><NoticeBar /><Hero {...props} /></>) : null} {fullWidth || props.post ? null : <PostHeader {...props} isDarkMode={isDarkMode} />} </header> ); const slotRight = router.route === '/404' || fullWidth ? null : <SideRight {...props} />; const maxWidth = fullWidth ? 'max-w-[96rem] mx-auto' : 'max-w-[86rem]'; useEffect(() => { loadWowJS() }, []); return ( <div id='theme-heo' className={`${siteConfig('FONT_STYLE')} bg-[#f7f9fe] dark:bg-[#18171d] h-full min-h-screen flex flex-col scroll-smooth`}> <Style /> {headerSlot} <main id='wrapper-outer' className={`flex-grow w-full ${maxWidth} mx-auto relative md:px-5`}> <div id='container-inner' className='w-full mx-auto lg:flex justify-center relative z-10'> <div className={`w-full h-auto ${className || ''}`}>{slotTop}{children}</div> <div className='lg:px-2'></div> <div className='hidden xl:block'>{slotRight}</div> </div> </main> <Footer /> {siteConfig('HEO_LOADING_COVER', true, CONFIG) && <LoadingCover />} </div> )};
const BottomNavBar = () => { const navItems = [ { href: '/', icon: 'fas fa-home', label: '主页', auth: false }, { href: '/ai-assistant', icon: 'fas fa-robot', label: 'AI助手', auth: false }, { href: '/community', icon: 'fas fa-users', label: '社区', auth: true }, { href: '/messages', icon: 'fas fa-comment-dots', label: '消息', auth: true }, { href: '/profile', icon: 'fas fa-user', label: '我', auth: true }, ]; const router = useRouter(); const { user, authLoading } = useAuth(); const [showLoginModal, setShowLoginModal] = useState(false); const handleLinkClick = (e, item) => { if (item.auth && !authLoading && !user) { e.preventDefault(); setShowLoginModal(true); } }; return ( <> <AuthModal show={showLoginModal} onClose={() => setShowLoginModal(false)} /> <nav className='fixed bottom-0 left-0 right-0 h-16 bg-white/80 dark:bg-black/80 backdrop-blur-lg shadow-[0_-2px_10px_rgba(0,0,0,0.1)] z-50 flex justify-around items-center'> {navItems.map(item => ( <SmartLink key={item.href} href={item.href} onClick={(e) => handleLinkClick(e, item)} className={`flex flex-col items-center justify-center w-1/5 ${authLoading && item.auth ? 'opacity-50 cursor-not-allowed' : ''}`}> <i className={`${item.icon} text-xl ${router.pathname === item.href ? 'text-blue-500' : 'text-gray-500'}`}></i> <span className={`text-xs mt-1 ${router.pathname === item.href ? 'text-blue-500' : 'text-gray-500'}`}>{item.label}</span> </SmartLink> ))} </nav> </> ); };
...
const LayoutIndex = props => {
  ...
  useEffect(() => {
    ...
    // --- 调试步驟 1: 檢查 Observer 是否被設置 ---
    console.log('useEffect is running, trying to set up IntersectionObserver.');

    const observer = new IntersectionObserver(
        ([entry]) => {
            // --- 调试步驟 2: 檢查 Observer 回調是否觸發 ---
            console.log('IntersectionObserver callback fired!', { isIntersecting: entry.isIntersecting });
            setIsCategoryBarSticky(!entry.isIntersecting);
        },
        { root: null, threshold: 0, rootMargin: '0px' } // 使用更簡單的配置來確保觸發
    );

    const currentSentinel = sentinelRef.current;
    if (currentSentinel) {
        console.log('Observer is now observing the sentinel element.');
        observer.observe(currentSentinel);
    } else {
        console.error('Sentinel element ref is not available!');
    }

    return () => {
        if (currentSentinel) {
            console.log('Observer is disconnecting.');
  ...
  const contentSwipeHandlers = useSwipeable({
    onSwipedLeft: () => { if (!isCategoryBarSticky) return; const currentIndex = tabs.findIndex(t => t.name === activeTab); if (currentIndex < tabs.length - 1) setActiveTab(tabs[currentIndex + 1].name); },
    onSwipedRight: () => { if (!isCategoryBarSticky) return; const currentIndex = tabs.findIndex(t => t.name === activeTab); if (currentIndex > 0) setActiveTab(tabs[currentIndex - 1].name); },
    onSwiping: (e) => { if (Math.abs(e.deltaX) > Math.abs(e.deltaY)) e.event.preventDefault(); },
  ...
  // --- 调试步驟 3: 在每次渲染時輸出當前狀態 ---
  console.log('Component rendered. isCategoryBarSticky:', isCategoryBarSticky);
        ...
        <div className='relative flex-grow w-full h-full'>
            ...
            <div className='absolute inset-0 z-20 overflow-y-auto overscroll-y-contain custom-scrollbar'>
                ...
                <div className='relative bg-white/95 dark:bg-gray-900/95 backdrop-blur-sm rounded-t-2xl shadow-2xl pb-16 min-h-[calc(55vh+1px)]'>
                    ...
                    {/* --- 调试步驟 4: 添加視覺反饋 --- */}
                    <div className={`sticky top-0 z-30 border-b border-t border-gray-200 dark:border-gray-700 transition-colors duration-300 ${isCategoryBarSticky ? 'bg-red-500' : 'bg-white/80 dark:bg-black/70 backdrop-blur-lg'}`}>
                        <div className='flex justify-around'>
                            {tabs.map(tab => (
                            <button key={tab.name} onClick={() => setActiveTab(tab.name)} className={`flex flex-col items-center justify-center w-1/5 pt-2.5 pb-1.5 transition-colors duration-300 focus:outline-none ${activeTab === tab.name ? 'text-blue-500' : 'text-gray-500 dark:text-gray-400'} ${isCategoryBarSticky && 'text-white'}`}>
                                {tab.icon}
                                <span className='text-xs font-semibold mt-1'>{tab.name}</span>
                                <div className={`w-6 h-0.5 mt-1 rounded-full transition-all duration-300 ${activeTab === tab.name ? (isCategoryBarSticky ? 'bg-white' : 'bg-blue-500') : 'bg-transparent'}`}></div>
                            </button>
                            ))}
                        </div>
                    </div>
