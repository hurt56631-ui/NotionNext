...
const LayoutBase = props => {
  ...
  // 首页将由 LayoutIndex 完全接管，所以这里对首页不做特殊处理，以保证其他页面正常
  if (router.route === '/') {
    return <>{children}</>
  }
...
const LayoutIndex = props => {
  ...
  const handlers = useSwipeable({
    ...
    preventDefaultTouchmoveEvent: true, // 关键：阻止 iframe 吞噬事件
    trackMouse: true,
    delta: 30 // 提高灵敏度 (数值越小越灵敏)
  });
  ...
  const renderContent = () => {
    switch (activeTab) {
        ...
        case 'HSK':
          return <iframe key="hsk" src="about:blank" title="HSK" style={{ width: '100%', height: 'calc(100vh - 100px)', border: 'none' }} />;
        case '口语':
          return <iframe key="kouyu" src="about:blank" title="口语" style={{ width: '100%', height: 'calc(100vh - 100px)', border: 'none' }} />;
        case '练习':
          return <iframe key="lianxi" src="about:blank" title="练习" style={{ width: '100%', height: 'calc(100vh - 100px)', border: 'none' }} />;
        default:
          return null;
      }
  };
  ...
  return (
    <div id='theme-heo' className={`${siteConfig('FONT_STYLE')} h-screen flex flex-col`}>
        <Style/>
        <Header {...props}/>
        <div 
            className='flex-grow relative w-full overflow-y-auto bg-cover bg-center' 
        ...
        >
            ...
            <div className='relative z-10 min-h-[65vh] bg-white dark:bg-gray-900 rounded-t-2xl shadow-2xl'>
                ...
                {/* 将手势绑定在稳定的父容器上 */}
                <main {...handlers} className="overflow-hidden">
