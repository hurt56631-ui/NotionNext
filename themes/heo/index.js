...
// --- 核心修改：引入 next/dynamic ---
import dynamic from 'next/dynamic'

// --- 动态加载新主页组件，并禁用SSR ---
// --- 这是修复 Error #130 的最终方案 ---
const DynamicNewHomePage = dynamic(
  () => import('./components/NewHomePage').then(mod => mod.default),
  {
    ssr: false,
...
const LayoutBase = props => {
  ...
  // 首页不显示默认的Header，因为它由新主页组件自己管理
  const showHeader = router.route !== '/'
  ...
  // 首页不显示右侧栏
  const showRightSlot = router.route !== '/' && router.route !== '/404' && !fullWidth
  const slotRight = showRightSlot ? <SideRight {...props} /> : null
  ...
  // 首页占据全宽，不应用maxWidth和padding
  const mainWrapperClass = router.route === '/'
    ? 'w-full'
    : `flex-grow w-full ${maxWidth} mx-auto relative md:px-5`
  ...
  return (
    <div
      ...
      <main id='wrapper-outer' className={mainWrapperClass}>
        <div
          id='container-inner'
...
const LayoutIndex = props => {
  return <DynamicNewHomePage {...props} />
}
