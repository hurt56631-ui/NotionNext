// /pages/messages/index.js (全新的全屏消息列表页面)
...
import { LayoutBase } from '@/themes/heo'; // 引入您的基础布局

// ------------------------------------------------------------------
// 1. MessageHeader 组件 (来自您的代码，直接放在这里)
// ------------------------------------------------------------------
...
// ------------------------------------------------------------------
// 2. ConversationList 组件 (用于显示私信列表)
// ------------------------------------------------------------------
const ConversationList = () => {
    ...
    const handleConversationClick = (peerUser) => {
        // 我们需要把对方的用户信息传递给聊天页面
        // 注意：这里我们不再使用 chatId，而是直接传递对方用户信息
        // 因为 PrivateChat 组件现在是根据 peerUid 来工作的
        router.push({
            pathname: `/chat/${peerUser.id}`,
            query: {
                peerDisplayName: peerUser.displayName,
                // peerPhotoURL: peerUser.photoURL // 如果需要头像，也可以传过去
            }
        });
    };
