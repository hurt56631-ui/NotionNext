// pages/community/[id].js （客户端延迟加载 Firebase，修复 "self is not defined"）
...
// ---------------------------
// 延迟动态导入 Firebase（客户端使用）
// ---------------------------
let fbCache = null;
const ensureFirebase = async () => {
  if (fbCache) return fbCache;
  // 动态导入 firestore 函数与你的 firebase 初始化模块（确保 '@/lib/firebase' 导出的是 client-side db 实例）
  const [
    firestoreModule,
    firebaseAppModule
  ] = await Promise.all([
  ...
  // 将需要的函数解构出来并缓存
  const { doc, getDoc, collection, query, where, orderBy, onSnapshot, addDoc, updateDoc, increment, serverTimestamp, runTransaction, writeBatch, deleteDoc, setDoc } = firestoreModule;
  const { db } = firebaseAppModule;

  fbCache = { doc, getDoc, collection, query, where, orderBy, onSnapshot, addDoc, updateDoc, increment, serverTimestamp, runTransaction, writeBatch, deleteDoc, setDoc, db };
  return fbCache;
};

// ---------------------------
// 辅助函数：视频解析、去 URL、AI 调用、TTS（保留原样）
// ---------------------------
...
// ---------------------------
// 子组件：CommentForm / CommentThread（与原版保持一致，稍作结构化）
// ---------------------------
...
const PostDetailPage = () => {
  ...
  // 监听帖子与评论（客户端动态 import Firestore）
  useEffect(() => {
    if (!id) return;
    setLoading(true);
    let postUnsub = null;
    ...
    ensureFirebase().then(fb => {
        if (cancelled) return;
        try {
            const postRef = fb.doc(fb.db, 'posts', id);
            postUnsub = fb.onSnapshot(postRef, (docSnap) => {
                if (docSnap.exists()) {
            ...
            const commentsQuery = fb.query(fb.collection(fb.db, 'comments'), fb.where('postId', '==', id), fb.orderBy('createdAt', 'asc'));
            commentsUnsub = fb.onSnapshot(commentsQuery, (snapshot) => {
        ...
        console.error("ensureFirebase failed:", err);
        setError('无法初始化数据库（请检查网络或配置）。');
        setLoading(false);
    });

    return () => {
        cancelled = true;
        try { if (postUnsub) postUnsub(); } catch(e) {}
        try { if (commentsUnsub) commentsUnsub(); } catch(e) {}
    };
  }, [id]);
  ...
  useEffect(() => {
      if(!id || !user) return;
      let mounted = true;
      ensureFirebase().then(async fb => {
          try {
              const followedDoc = await fb.getDoc(fb.doc(fb.db, 'users', user.uid, 'following', post?.authorId || ''));
              if (mounted) setIsFollowing(followedDoc.exists());
          } catch (e) { /* ignore */ }
          try {
              const favDoc = await fb.getDoc(fb.doc(fb.db, 'users', user.uid, 'favorites', id));
              if (mounted) setIsFavorited(favDoc.exists());
          } catch (e) { /* ignore */ }
      }).catch(e => console.error("ensureFirebase for follow/fav failed:", e));
      return () => { mounted = false; };
  }, [id, user, post?.authorId]);

  // ---------- DB 操作处理函数：均动态加载 firestore 并使用 ----------
  const handleVote = async (type) => {
    ...
    try {
        ...
        await fb.runTransaction(fb.db, async (t) => {
            ...
            const hasLikedLocal = likers.includes(user.uid);
            const hasDislikedLocal = dislikers.includes(user.uid);
            ...
            t.update(postRef, {
                ...
                likesCount: fb.increment(likesIncrement),
                dislikesCount: fb.increment(dislikesIncrement)
            });
        });
    } catch (e) {
  ...
  const handleFollow = async () => {
    ...
    try {
        ...
        const followingRef = fb.doc(fb.db, 'users', user.uid, 'following', post.authorId);
        const followerRef = fb.doc(fb.db, 'users', post.authorId, 'followers', user.uid);
        if (isFollowing) {
            batch.delete(followingRef);
            batch.delete(followerRef);
        } else {
            batch.set(followingRef, { followedAt: fb.serverTimestamp() });
            batch.set(followerRef, { followerAt: fb.serverTimestamp() });
        }
        await batch.commit();
  ...
  const handleFavorite = async () => {
    ...
    try {
        const fb = await ensureFirebase();
        const favRef = fb.doc(fb.db, 'users', user.uid, 'favorites', id);
        if (isFavorited) {
            await fb.deleteDoc(favRef);
        } else {
            await fb.setDoc(favRef, { postId: id, title: post.title, favoritedAt: fb.serverTimestamp() });
        }
  ...
  const handleCommentSubmit = async (content, parentId = null) => {
    ...
    try {
        const fb = await ensureFirebase();
        await fb.addDoc(fb.collection(fb.db, 'comments'), { postId: id, content, parentId, authorId: user.uid, authorName: user.displayName || '匿名用户', authorAvatar: user.photoURL, createdAt: fb.serverTimestamp(), likesCount: 0, likers: [] });
        if (!parentId) { await fb.updateDoc(fb.doc(fb.db, 'posts', id), { commentsCount: fb.increment(1) }); }
    } catch (e) {
  ...
  const handleDeleteComment = async (commentId) => {
    ...
    if (!commentToDelete || user?.uid !== commentToDelete.authorId) { alert("无权删除。"); return; }
    if (window.confirm("确定删除评论吗？")) {
        try {
            const fb = await ensureFirebase();
            await fb.deleteDoc(fb.doc(fb.db, 'comments', commentId));
            if (!commentToDelete.parentId) { await fb.updateDoc(fb.doc(fb.db, 'posts', id), { commentsCount: fb.increment(-1) }); }
        } catch (e) {
  ...
  const handleLikeComment = async (commentId) => {
    ...
    try {
        const fb = await ensureFirebase();
        const commentRef = fb.doc(fb.db, 'comments', commentId);
        await fb.runTransaction(fb.db, async (transaction) => {
            ...
            const hasLikedLocal = likers.includes(user.uid);
            if (hasLikedLocal) {
                transaction.update(commentRef, { likers: likers.filter(uid => uid !== user.uid), likesCount: fb.increment(-1) });
            } else {
                transaction.update(commentRef, { likers: [...likers, user.uid], likesCount: fb.increment(1) });
            }
        });
    } catch (e) {
