// components/ShortSentenceCard.js (功能合体最终版)
...
const ShortSentenceCard = ({ sentences = [], isOpen, onClose }) => {
  ...
  // 将传入的 sentences prop 转换为内部使用的 cards 格式
  const processedCards = useMemo(() => {
    try {
        const mapped = sentences.map(s => ({
            ...
            pinyin: s.pinyin,
            imageUrl: s.imageUrl, // 支持可选的 imageUrl
        }));
    ...
    item && (
      <animated.div style={{ ...styles.fullScreen, ...style }}>
        ...
        <div style={styles.headerControls} data-no-gesture="true">
            <button style={styles.headerButton} onClick={onClose}><FaTimes size={22} /></button>
...
const styles = {
    ...
    cardContainer: { width: '100%', height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'center', background: 'rgba(255, 255, 255, 0.85)', backdropFilter: 'blur(15px)', borderRadius: '24px', boxShadow: '0 20px 40px -10px rgba(0, 30, 80, 0.15)', overflow: 'hidden' },
    ...
    translationContent: { flex: 2, display: 'flex', justifyContent: 'center', alignItems: 'center', background: 'rgba(248, 250, 252, 0.7)', borderTop: '1px solid rgba(226, 232, 240, 0.8)', padding: '20px', cursor: 'pointer' },
    pinyin: { fontSize: '1.5rem', color: '#64748b', marginBottom: '1rem', letterSpacing: '0.05em' },
    textChinese: { fontSize: '3.2rem', fontWeight: 'bold', color: '#1e293b', lineHeight: 1.4, wordBreak: 'break-word' },
    textBurmese: { fontSize: '2.5rem', color: '#312e81', fontFamily: '"Padauk", "Myanmar Text", sans-serif', lineHeight: 1.8, wordBreak: 'break-word' },

    // --- 控件 ---
    headerControls: { position: 'fixed', top: '25px', left: '25px', right: '25px', zIndex: 10, display: 'flex', justifyContent: 'space-between', alignItems: 'center' },
    headerButton: { background: 'rgba(255, 255, 255, 0.7)', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', width: '48px', height: '48px', borderRadius: '50%', backdropFilter: 'blur(5px)' },
