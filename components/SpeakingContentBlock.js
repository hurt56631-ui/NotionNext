...
const SpeakingContentBlock = () => {
  ...
  const handleCourseClick = async (courseSummary) => {
    ...
    try {
      // 假设这些文件放在 public/data/lessons/ 目录下
  ...
  useEffect(() => {
    const handleHashChange = () => {
      ...
      if (hash.includes('#course-vocab')) setActiveModule('vocab');
          ...
          setActiveModule(null); // 回到菜单，但不清除 selectedCourse
      }
      else { 
          // 这里的 hash 既不是 module 也不是 menu，说明完全退出了
  ...
  return (
    <>
      ...
      {/* ✅ 修改点：直接渲染全屏互动组件，去除多余的 wrapper 和 button */}
