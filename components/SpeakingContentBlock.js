...
import { createPortal } from 'react-dom'; // 👈 重新引入 Portal
...
// ✅ 新增：专门的全屏容器
// 这个容器负责把里面的东西（比如 InteractiveLesson）强制全屏显示
...
const SpeakingContentBlock = () => {
  ...
  const handleCourseClick = async (courseSummary) => {
    ...
    const fetchSafe = async (url) => { try { const res = await fetch(url); return res.ok ? await res.json() : []; } catch (e) { return []; } };
    ...
    } catch (error) { console.error(error); alert("加载课程失败"); } finally { setIsLoading(false); }
  };

  // 2. 状态路由同步
  const handleModuleClick = (type) => { setActiveModule(type); router.push(router.asPath.split('#')[0] + `#course-${type}`, undefined, { shallow: true }); };
  ...
  const transformToWordStudyLesson = (data, title, isSentence = false) => {
    ...
    return { blocks: [{ type: "word_study", content: { title: title, words: data.map(item => ({ id: item.id, chinese: isSentence ? item.sentence : item.word, pinyin: item.pinyin, translation: item.translation, example: item.example, rate: isSentence ? 0.85 : 0 })) } }, { type: "complete", content: { title: "学习完成！" } }] };
  };
  const transformGrammarToLesson = (data) => {
    if (!data || data.length === 0) return { blocks: [] };
    return { blocks: [{ type: "grammar_study", content: { grammarPoints: data.map(g => ({ id: g.id, grammarPoint: g.sentence || g.pattern, pattern: g.pattern || g.sentence, visibleExplanation: g.visibleExplanation || `<div class="font-bold text-blue-600 mb-2">${g.translation || ''}</div><div>${g.explanation || ''}</div>` + (g.usage||''), examples: g.examples || [] })) } }, { type: "complete", content: { title: "语法通关！" } }] };
  };
    ...
    <>
      {isLoading && (
        <div className="fixed inset-0 z-[50] bg-black/20 backdrop-blur-sm flex items-center justify-center">
            <div className="bg-white p-4 rounded-xl shadow-xl flex items-center gap-3"><Loader2 className="animate-spin text-teal-600" /><span className="font-medium">资源加载中...</span></div>
        </div>
      )}

      {/* 课程列表 */}
      <div className="space-y-4 pb-20">
        <div className="text-center mb-6 pt-4"><h2 className="text-2xl font-bold text-gray-800 dark:text-gray-100">口语速成</h2><p className="text-sm text-gray-500">共 {speakingList.length} 课精选内容</p></div>
      ...
      <AnimatePresence>
        {selectedCourse && !activeModule && (
          <motion.div initial={{ opacity: 0, y: "100%" }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: "100%" }} transition={{ type: "spring", damping: 25, stiffness: 200 }} className="fixed inset-0 z-[40] bg-gray-50 dark:bg-gray-900 flex flex-col">
            <div className="p-4 bg-white dark:bg-gray-800 shadow-sm flex items-center z-10"><button onClick={handleBack} className="p-2 -ml-2 text-gray-600 dark:text-gray-300 active:bg-gray-100 rounded-full"><ChevronRight className="rotate-180" size={24}/></button><h2 className="flex-1 text-center font-bold text-lg pr-8 truncate">{selectedCourse.title}</h2></div>
      ...
      {/* ✅✅✅ 这里的改变：用 FullScreenWrapper 包裹 InteractiveLesson */}
      {/* 这样 InteractiveLesson 只需要负责显示，而不用管怎么全屏 */}
...
// 菜单卡片组件
const MenuCard = ({ title, subtitle, icon, color, onClick }) => (<div onClick={onClick} className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 flex items-center gap-6 active:scale-95 transition-transform cursor-pointer hover:shadow-md"><div className={`w-14 h-14 rounded-full ${color} text-white flex items-center justify-center shadow-lg shrink-0`}>{icon}</div><div className="flex-1"><h3 className="text-xl font-bold text-gray-800 dark:text-gray-100">{title}</h3><p className="text-gray-500 dark:text-gray-400 text-sm mt-1">{subtitle}</p></div><ChevronRight className="ml-auto text-gray-300" /></div>);
