// AIChatDrawer.js (最终完美版 - 高质量流式朗读、划词菜单修复、API 修正)

import { Transition } from '@headlessui/react'
import React, { useState, useEffect, useRef, useCallback, useMemo, Fragment } from 'react';
import imageCompression from 'browser-image-compression';
import { pinyin } from 'pinyin-pro'; // 确保安装: npm install pinyin-pro
...
// 2. 组件映射表
const componentMap = { PaiXuTi: PaiXuTi };
...
class AudioQueue {
    ...
    process() {
        ...
        this.currentAudio.onerror = () => { console.error("Audio playback error"); this.isPlaying = false; this.currentAudio = null; this.process(); };
        this.currentAudio.play().catch((e) => { console.warn("Autoplay was prevented.", e); this.isPlaying = false; this.process(); });
    }
    clear() {
        if (this.currentAudio) {
            this.currentAudio.pause();
            this.currentAudio.src = ''; // 强制停止加载
            this.currentAudio = null;
        }
...
const fetchEdgeTTS = async (text, voice, rate, pitch) => {
    try {
        ...
        const ssml = `<speak version='1.0' xmlns='http://www.w3.org/2001/10/synthesis' xml:lang='en-US'><voice name='${voice}'><prosody pitch='${pitchStr}' rate='${rateStr}'>${text.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')}</prosody></voice></speak>`;
...
// --- 其他工具函数 ---
const sanitizeQuizData = (props) => { /* ... 保持不变 ... */ if (!props || !props.items || !props.correctOrder) return props; let items = [...props.items]; const correctOrder = [...props.correctOrder]; const punctuationRegex = /^[。，、？！；：“”‘’（）《》〈〉【】 .,!?;:"'()\[\]{}]+$/; const orphanPunctuationItems = items.filter(item => { const isPunctuation = punctuationRegex.test(item.content.trim()); const isInCorrectOrder = correctOrder.includes(item.id); return isPunctuation && !isInCorrectOrder; }); if (orphanPunctuationItems.length > 0 && correctOrder.length > 0) { const lastWordId = correctOrder[correctOrder.length - 1]; let lastWordIndex = items.findIndex(item => item.id === lastWordId); if (lastWordIndex !== -1) { const newContent = orphanPunctuationItems.reduce((acc, puncItem) => acc + puncItem.content, items[lastWordIndex].content); items[lastWordIndex] = { ...items[lastWordIndex], content: newContent }; } const orphanIds = new Set(orphanPunctuationItems.map(item => item.id)); items = items.filter(item => !orphanIds.has(item.id)); } return { ...props, items: items, correctOrder: correctOrder };};
...
const CHAT_MODELS_LIST = [ { id: 'model-1', name: 'Gemini 2.5 Flash', value: 'gemini-2.5-flash', maxContextTokens: 1048576 }, { id: 'model-2', name: 'Gemini 2.5 Pro', value: 'gemini-2.5-pro', maxContextTokens: 1048576 }, { id: 'model-3', name: 'Gemini 2.0 Flash', value: 'gemini-2.0-flash', maxContextTokens: 1048576 }, { id: 'model-4', name: 'Gemini 1.5 Flash', value: 'gemini-1.5-flash-latest', maxContextTokens: 1048576 }, { id: 'model-5', name: 'Gemini 1.5 Pro', value: 'gemini-1.5-pro-latest', maxContextTokens: 1048576 }, ];
const DEFAULT_PROMPTS = [ { id: 'default-grammar-correction', name: '纠正中文语法', description: '纠正语法、优化用词。', content: '你是一位专业的中文老师。请纠正用户发送的中文句子中的语法错误。\n\n**重要要求**：\n1. 请务必使用 **Markdown** 格式。\n2. 错误的地方请用 `代码块` 或 **加粗** 标记。\n3. 使用列表展示修改建议。', openingLine: '你好，请发送你需要我纠正的中文句子。', model: 'gemini-2.5-flash', ttsVoice: 'zh-CN-XiaoxiaoMultilingualNeural', avatarUrl: '' }, { id: 'explain-word', name: '解释中文词语', description: '解释词语并造句。', content: '你是一位中文老师。请解释用户发送的中文词语。\n\n**格式要求**：\n- 使用 ### 标题 分隔含义和例句。\n- 关键解释请 **加粗**。', openingLine: '你好，想了解哪个词？', model: 'gemini-1.5-pro-latest', ttsVoice: 'zh-CN-YunxiNeural', avatarUrl: '' }, { id: 'translate-myanmar', name: '中缅互译', description: '中缅互译助手。', content: '你是一位翻译助手。请将用户发送的内容在中文和缅甸语之间互译。请直接输出翻译结果。', openingLine: '你好！请发送内容进行翻译。', model: 'gemini-2.5-flash', ttsVoice: 'my-MM-NilarNeural', avatarUrl: '' } ];
const DEFAULT_SETTINGS = { apiKey: '', apiKeys: [], activeApiKeyId: '', chatModels: CHAT_MODELS_LIST, selectedModel: 'gemini-2.5-flash', temperature: 0.8, maxOutputTokens: 8192, disableThinkingMode: true, startWithNewChat: false, prompts: DEFAULT_PROMPTS, currentPromptId: DEFAULT_PROMPTS[0]?.id || '', autoRead: true, voiceAutoSend: false, ttsEngine: TTS_ENGINE.THIRD_PARTY, ttsVoice: 'zh-CN-XiaoxiaoMultilingualNeural', ttsRate: 0, ttsPitch: 0, systemTtsVoiceURI: '', speechLanguage: 'zh-CN', chatBackgroundUrl: '/images/chat-bg-light.jpg', backgroundOpacity: 70, userAvatarUrl: 'https://raw.githubusercontent.com/Flipped-Development/images/main/user-avatar-default.png', aiAvatarUrl: 'https://raw.githubusercontent.com/Flipped-Development/images/main/gemini-sparkle-animated.gif', isFacebookApp: false, };
...
const TypingEffect = ({ text, settings, onComplete, onUpdate }) => {
            ...
            const promises = sentences.map(async (sentence, index) => {
                ...
                if (cleanText) {
                    ...
                    if (settings.ttsEngine === TTS_ENGINE.THIRD_PARTY) {
                        ...
                        audioUrl = `system:${cleanText}`; // 系统语音标记
                    }
    ...
    useEffect(() => {
        ...
        const intervalId = setInterval(() => {
            ...
            if (charIndex >= (sentences.slice(0, sentenceIndex + 1).join('').length - 1)) {
...
const TextActionMenu = () => {
    ...
    const handleSelection = useCallback(() => {
        ...
        if (text && text.length > 0) {
            ...
            setMenuStyle({ display: 'flex', top: `${window.scrollY + rect.top - 60}px`, left: `${window.scrollX + rect.left + rect.width / 2 - 100}px` });
    ...
    return (
        <div className="fixed z-50 bg-gray-900/90 backdrop-blur-md text-white rounded-xl shadow-2xl flex flex-col items-center p-1 animate-fade-in border border-gray-700" style={menuStyle} onMouseDown={(e) => e.preventDefault()}>
...
const ThinkingIndicator = ({ settings, aiAvatar }) => ( <div className="flex items-end gap-3 my-5 justify-start"> <img src={convertGitHubUrl(aiAvatar || settings.aiAvatarUrl)} className="w-9 h-9 rounded-full shadow-sm" /> <div className="px-4 py-3 rounded-2xl rounded-tl-sm bg-white dark:bg-gray-700 border border-gray-100 shadow-sm flex items-center gap-1"> <div className="w-2 h-2 bg-blue-400 rounded-full animate-bounce [animation-delay:-0.3s]"></div> <div className="w-2 h-2 bg-blue-400 rounded-full animate-bounce [animation-delay:-0.15s]"></div> <div className="w-2 h-2 bg-blue-400 rounded-full animate-bounce"></div> </div> </div> );
const ChatSidebar = ({ isOpen, conversations, currentId, onSelect, onNew, onDelete, onRename, prompts, settings }) => { /* ... 保持不变 ... */ const [editingId, setEditingId] = useState(null); const [newName, setNewName] = useState(''); const handleRename = (id, oldName) => { setEditingId(id); setNewName(oldName); }; const handleSaveRename = (id) => { if (newName.trim()) onRename(id, newName.trim()); setEditingId(null); }; const grouped = useMemo(() => { const groups = new Map(); const uncategorized = []; (conversations || []).forEach(conv => { const prompt = (prompts || []).find(p => p.id === conv.promptId); if (prompt) { if (!groups.has(prompt.id)) groups.set(prompt.id, { prompt, conversations: [] }); groups.get(prompt.id).conversations.push(conv); } else uncategorized.push(conv); }); return { sortedGroups: Array.from(groups.values()), uncategorized }; }, [conversations, prompts]); const renderItem = (conv) => ( <div key={conv.id} className={`flex items-center p-2.5 rounded-lg cursor-pointer transition-colors mb-1 ${currentId === conv.id ? 'bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400' : 'hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-700 dark:text-gray-300'}`} onClick={() => onSelect(conv.id)}> <div className="flex-grow truncate text-sm font-medium" onDoubleClick={(e) => {e.stopPropagation(); handleRename(conv.id, conv.title)}}> {editingId === conv.id ? <input autoFocus className="bg-transparent border-b border-blue-500 w-full outline-none" value={newName} onChange={e=>setNewName(e.target.value)} onBlur={()=>handleSaveRename(conv.id)} onKeyDown={e=>e.key==='Enter'&&handleSaveRename(conv.id)}/> : conv.title} </div> {currentId === conv.id && <div className="flex gap-1 ml-2"><button onClick={(e)=>{e.stopPropagation(); handleRename(conv.id, conv.title)}} className="p-1 hover:text-blue-600"><i className="fas fa-pen text-xs"></i></button><button onClick={(e)=>{e.stopPropagation(); if(confirm('删除?')) onDelete(conv.id)}} className="p-1 hover:text-red-500"><i className="fas fa-trash text-xs"></i></button></div>} </div> ); return ( <div className={`absolute lg:relative h-full bg-white/95 dark:bg-gray-900/95 backdrop-blur flex flex-col transition-all duration-300 z-30 ${isOpen ? 'w-64 border-r border-gray-200 dark:border-gray-800' : 'w-0 overflow-hidden'}`}> <div className="p-4"><button onClick={onNew} className="w-full py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold shadow-md transition-all active:scale-95 flex items-center justify-center gap-2"><i className="fas fa-plus"></i> 新对话</button></div> <div className="flex-1 overflow-y-auto px-3 pb-4"> {grouped.sortedGroups.map(({ prompt, conversations }) => ( <details key={prompt.id} open className="group mb-2"> <summary className="flex items-center gap-2 px-2 py-1.5 text-xs font-bold text-gray-400 uppercase tracking-wider cursor-pointer hover:text-gray-600 list-none"><i className="fas fa-chevron-right text-[10px] transition-transform group-open:rotate-90"></i> {prompt.name}</summary> <div className="pl-2 mt-1">{conversations.map(renderItem)}</div> </details> ))} {grouped.uncategorized.length > 0 && <details open className="group"><summary className="px-2 py-1.5 text-xs font-bold text-gray-400 uppercase cursor-pointer list-none">未分类</summary><div className="pl-2 mt-1">{grouped.uncategorized.map(renderItem)}</div></details>} </div> </div> ); };
const SubPageWrapper = ({ title, onBack, onSave, children }) => ( <div className="flex flex-col h-full bg-white dark:bg-gray-800"> <div className="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-700"> <h3 className="text-xl font-bold text-gray-800 dark:text-white">{title}</h3> <button onClick={onBack} className="w-8 h-8 flex items-center justify-center rounded-full bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300"><i className="fas fa-times"></i></button> </div> <div className="flex-grow overflow-y-auto p-4 space-y-4 pb-20">{children}</div> <div className="p-4 border-t border-gray-100 dark:border-gray-700 bg-white/80 dark:bg-gray-800/80 backdrop-blur-md sticky bottom-0 z-10 flex justify-end gap-3 pb-safe"> <button onClick={onBack} className="px-5 py-2.5 rounded-xl font-medium text-gray-600 bg-gray-100 hover:bg-gray-200 dark:bg-gray-700 dark:text-gray-200">返回</button> <button onClick={onSave} className="px-5 py-2.5 rounded-xl font-bold text-white bg-blue-600 shadow-lg shadow-blue-500/30 hover:bg-blue-700">保存设置</button> </div> </div> );
const PromptManager = ({ prompts, onChange, onAdd, onDelete, settings }) => { /* ... 保持不变 ... */ const handleAvatarUpload = async (file, promptId) => { try { const compressedFile = await imageCompression(file, { maxSizeMB: 0.5, maxWidthOrHeight: 512 }); const reader = new FileReader(); reader.onload = (e) => onChange(promptId, 'avatarUrl', e.target.result); reader.readAsDataURL(compressedFile); } catch (err) { alert('图片上传失败'); } }; return ( <> {(prompts || []).map(p => ( <div key={p.id} className="p-4 bg-gray-50 dark:bg-gray-700/50 rounded-2xl border border-gray-200 dark:border-gray-600 space-y-3"> <div className="flex items-start gap-4"> <div className="relative shrink-0 group"> <img src={convertGitHubUrl(p.avatarUrl) || convertGitHubUrl(settings.aiAvatarUrl)} alt="" className="w-16 h-16 rounded-2xl object-cover shadow-sm bg-white" /> <label className="absolute inset-0 flex items-center justify-center bg-black/40 rounded-2xl opacity-0 group-hover:opacity-100 cursor-pointer transition-opacity"> <i className="fas fa-camera text-white"></i> <input type="file" accept="image/*" className="hidden" onChange={(e) => e.target.files && handleAvatarUpload(e.target.files, p.id)} /> </label> </div> <div className="flex-grow space-y-2"> <input type="text" value={p.name} onChange={(e) => onChange(p.id, 'name', e.target.value)} className="w-full bg-transparent font-bold text-lg border-b border-transparent hover:border-gray-300 focus:border-blue-500 focus:outline-none transition-colors" placeholder="助理名称" /> <input type="text" value={p.description || ''} onChange={(e) => onChange(p.id, 'description', e.target.value)} className="w-full bg-transparent text-sm text-gray-500 border-b border-transparent hover:border-gray-300 focus:border-blue-500 focus:outline-none transition-colors" placeholder="一句话简介..." /> </div> {!p.id.startsWith('default-') && <button onClick={() => onDelete(p.id)} className="p-2 text-gray-400 hover:text-red-500"><i className="fas fa-trash"></i></button>} </div> {p.id.startsWith('default-') ? (<div className="p-3 bg-gray-100 dark:bg-gray-600 rounded-xl text-xs text-gray-500 italic text-center">系统内置提示词，不可修改核心内容</div>) : (<textarea value={p.content} onChange={(e) => onChange(p.id, 'content', e.target.value)} placeholder="系统提示词..." className="w-full h-24 p-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-xl text-sm outline-none resize-none" />)} <input type="text" value={p.openingLine || ''} onChange={(e) => onChange(p.id, 'openingLine', e.target.value)} placeholder="开场白..." className="w-full p-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-xl text-sm outline-none" /> </div> ))} <button onClick={onAdd} className="w-full py-4 font-bold text-blue-600 bg-blue-50 dark:bg-blue-900/20 rounded-2xl border-2 border-dashed border-blue-200 dark:border-blue-800 hover:bg-blue-100 dark:hover:bg-blue-900/40 transition-colors"><i className="fas fa-plus mr-2"></i> 创建新助理</button> </> ); };
const ModelManager = ({ models, onChange, onAdd, onDelete }) => { /* ... 保持不变 ... */ return( <> {(models || []).map(m => ( <div key={m.id} className="p-3 bg-gray-50 dark:bg-gray-700 rounded-md border border-gray-200 dark:border-gray-600 space-y-2"> <div className="flex items-center justify-between"> <input type="text" value={m.name} onChange={(e) => onChange(m.id, 'name', e.target.value)} placeholder="模型显示名称" className="font-semibold bg-transparent w-full text-lg" /> <button onClick={() => onDelete(m.id)} className="p-2 ml-2 text-sm text-red-500 rounded-full hover:bg-red-500/10"><i className="fas fa-trash"></i></button> </div> <div className="grid grid-cols-2 gap-2 text-sm"> <div> <label className="text-xs font-medium">模型值 (Value)</label> <input type="text" value={m.value} onChange={(e) => onChange(m.id, 'value', e.target.value)} placeholder="例如: gemini-1.5-pro-latest" className="w-full mt-1 px-2 py-1 bg-white dark:bg-gray-800 border dark:border-gray-500 rounded-md text-xs" /> </div> <div> <label className="text-xs font-medium">最大上下文 (Tokens)</label> <input type="number" value={m.maxContextTokens} onChange={(e) => onChange(m.id, 'maxContextTokens', parseInt(e.target.value, 10) || 0)} placeholder="例如: 8192" className="w-full mt-1 px-2 py-1 bg-white dark:bg-gray-800 border dark:border-gray-500 rounded-md text-xs" /> </div> </div> </div> ))} <button onClick={onAdd} className="w-full mt-4 py-3 bg-blue-500 text-white rounded-md shrink-0 mb-20"><i className="fas fa-plus mr-2"></i>添加新模型</button> </> );};
const ApiKeyManager = ({ apiKeys, activeApiKeyId, onChange, onAdd, onDelete, onSetActive }) => ( <><div className="mb-6 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-xl border border-blue-100 dark:border-blue-800"><h4 className="font-bold text-blue-800 dark:text-blue-300 mb-2"><i className="fas fa-info-circle mr-2"></i>关于 API 接口</h4><p className="text-sm text-blue-700 dark:text-blue-400 mb-2">选择 "OpenAI 兼容" 可使用第三方中转 (如 open-gemini-api.deno.dev)，无需 VPN。</p></div> {(apiKeys || []).map(k => (<div key={k.id} className={`p-4 rounded-xl border-2 transition-all duration-200 ${activeApiKeyId === k.id ? 'border-green-500 bg-green-50 dark:bg-green-900/10' : 'border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800'}`}><div className="flex items-center justify-between mb-3"><div className="flex items-center gap-2"><select value={k.provider} onChange={(e) => onChange(k.id, 'provider', e.target.value)} className="font-bold bg-transparent text-gray-700 dark:text-gray-200 border-none outline-none"><option value="gemini">Google 官方 (需VPN)</option><option value="openai">OpenAI 兼容 (第三方)</option></select></div><div className="flex gap-2">{activeApiKeyId !== k.id && <button onClick={() => onSetActive(k.id)} className="px-3 py-1 text-xs font-bold text-green-600 bg-green-100 rounded-lg hover:bg-green-200">启用</button>}<button onClick={() => onDelete(k.id)} className="p-2 text-red-400 hover:text-red-600"><i className="fas fa-trash"></i></button></div></div><div className="space-y-3"><input type="password" value={k.key} onChange={(e) => onChange(k.id, 'key', e.target.value)} placeholder="输入 API Key (sk-...)" className="w-full p-3 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg text-sm font-mono focus:ring-2 focus:ring-blue-500 outline-none" />{k.provider === 'openai' && (<input type="text" value={k.url || ''} onChange={(e) => onChange(k.id, 'url', e.target.value)} placeholder="默认: https://open-gemini-api.deno.dev/v1/chat/completions" className="w-full p-3 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg text-xs text-gray-500 focus:ring-2 focus:ring-blue-500 outline-none" />)}</div></div>))} <button onClick={onAdd} className="w-full py-4 font-bold text-indigo-600 bg-indigo-50 dark:bg-indigo-900/20 rounded-2xl border-2 border-dashed border-indigo-200 dark:border-indigo-800 hover:bg-indigo-100 dark:hover:bg-indigo-900/40 transition-colors"><i className="fas fa-plus mr-2"></i> 添加新密钥</button></> );
const SettingsModal = ({ settings, onSave, onClose }) => { const [tempSettings, setTempSettings] = useState(settings); const [systemVoices, setSystemVoices] = useState([]); const [view, setView] = useState('main'); const fileInputRef = useRef(null); const userAvatarInputRef = useRef(null); useEffect(() => { const fetchSystemVoices = () => { if (!window.speechSynthesis) return; const voices = window.speechSynthesis.getVoices(); if (voices.length > 0) { setSystemVoices(voices.filter(v => v.lang.startsWith('zh') || v.lang.startsWith('en') || v.lang.startsWith('fr') || v.lang.startsWith('es') || v.lang.startsWith('ja') || v.lang.startsWith('ko') || v.lang.startsWith('vi'))); } }; if (window.speechSynthesis) { if (window.speechSynthesis.onvoiceschanged !== undefined) { window.speechSynthesis.onvoiceschanged = fetchSystemVoices; } fetchSystemVoices(); } }, []); const handleChange = (key, value) => setTempSettings(prev => ({ ...prev, [key]: value })); const handleBgImageSelect = (event) => { const file = event.target.files; if (file && file.type.startsWith('image/')) { const reader = new FileReader(); reader.onload = (e) => { handleChange('chatBackgroundUrl', e.target.result); }; reader.readAsDataURL(file); } event.target.value = null; }; const handleUserAvatarSelect = (event) => { const file = event.target.files; if (file && file.type.startsWith('image/')) { const reader = new FileReader(); reader.onload = (e) => { handleChange('userAvatarUrl', e.target.result); }; reader.readAsDataURL(file); } event.target.value = null; }; const handleAddPrompt = () => { const newPrompt = { id: generateSimpleId('prompt'), name: '新助理', description: '新助理', content: '你是一个...', openingLine: '你好', model: settings.selectedModel, ttsVoice: 'zh-CN-XiaoxiaoMultilingualNeural', avatarUrl: '' }; const newPrompts = [...(tempSettings.prompts || []), newPrompt]; handleChange('prompts', newPrompts); }; const handleDeletePrompt = (idToDelete) => { if (!window.confirm('确定删除吗？')) return; const newPrompts = (tempSettings.prompts || []).filter(p => p.id !== idToDelete); handleChange('prompts', newPrompts); if (tempSettings.currentPromptId === idToDelete) handleChange('currentPromptId', newPrompts?.id || ''); }; const handlePromptSettingChange = (promptId, field, value) => { const newPrompts = (tempSettings.prompts || []).map(p => p.id === promptId ? { ...p, [field]: value } : p); handleChange('prompts', newPrompts); }; const speechLanguageOptions = [ { name: '中文 (普通话)', value: 'zh-CN' }, { name: '缅甸语 (မြန်မာ)', value: 'my-MM' }, { name: 'English (US)', value: 'en-US' }, { name: 'Español (España)', value: 'es-ES' }, { name: 'Français (France)', value: 'fr-FR' }, { name: '日本語', value: 'ja-JP' }, { name: '한국어', value: 'ko-KR' }, { name: 'Tiếng Việt', value: 'vi-VN' }, ]; const handleAddModel = () => { const newModel = { id: generateSimpleId('model'), name: '新模型', value: '', maxContextTokens: 8192 }; const newModels = [...(tempSettings.chatModels || []), newModel]; handleChange('chatModels', newModels); }; const handleDeleteModel = (idToDelete) => { if (!window.confirm('确定删除吗？')) return; const newModels = (tempSettings.chatModels || []).filter(m => m.id !== idToDelete); handleChange('chatModels', newModels); }; const handleModelSettingChange = (modelId, field, value) => { const newModels = (tempSettings.chatModels || []).map(m => m.id === modelId ? { ...m, [field]: value } : m); handleChange('chatModels', newModels); }; const handleAddApiKey = () => { const newKey = { id: generateSimpleId('key'), provider: 'openai', key: '', url: 'https://open-gemini-api.deno.dev/v1/chat/completions' }; const newKeys = [...(tempSettings.apiKeys || []), newKey]; handleChange('apiKeys', newKeys); if (newKeys.length === 1) { handleChange('activeApiKeyId', newKey.id); } }; const handleDeleteApiKey = (idToDelete) => { if (!window.confirm('确定删除吗？')) return; const newKeys = (tempSettings.apiKeys || []).filter(k => k.id !== idToDelete); handleChange('apiKeys', newKeys); if (tempSettings.activeApiKeyId === idToDelete) handleChange('activeApiKeyId', newKeys?.id || ''); }; const handleApiKeySettingChange = (keyId, field, value) => { const newKeys = (tempSettings.apiKeys || []).map(k => k.id === keyId ? { ...k, [field]: value } : k); handleChange('apiKeys', newKeys); }; const handleSetActiveApiKey = (keyId) => { handleChange('activeApiKeyId', keyId); }; const handleSubPageSave = () => { onSave(tempSettings); }; const MenuItem = ({ title, icon, onClick, color = "blue" }) => ( <button type="button" onClick={onClick} className={`w-full flex items-center p-4 mb-3 rounded-2xl bg-${color}-50 dark:bg-gray-700/50 border border-${color}-100 dark:border-gray-600 hover:bg-${color}-100 dark:hover:bg-gray-600 transition-all shadow-sm active:scale-98`}> <div className={`w-10 h-10 rounded-full bg-white dark:bg-gray-600 flex items-center justify-center text-${color}-500 shadow-sm mr-4`}><i className={`fas ${icon} text-lg`}></i></div> <span className="text-lg font-bold text-gray-800 dark:text-white flex-grow text-left">{title}</span> <i className="fas fa-chevron-right text-gray-400"></i> </button> ); return ( <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-end sm:items-center justify-center z-50 animate-fade-in" onClick={onClose}> <div className="bg-white dark:bg-gray-800 rounded-t-3xl sm:rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden relative text-gray-800 dark:text-gray-200 h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}> {view === 'main' && ( <div className="h-full flex flex-col"> <div className="p-6 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between shrink-0"><h3 className="text-2xl font-black bg-clip-text text-transparent bg-gradient-to-r from-blue-600 to-purple-600">设置中心</h3><button onClick={onClose} className="w-8 h-8 rounded-full bg-gray-100 dark:bg-gray-700 flex items-center justify-center"><i className="fas fa-times"></i></button></div> <div className="flex-grow overflow-y-auto p-6 space-y-6"> <div><h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">账号与模型</h4><MenuItem title="API 密钥管理" icon="fa-key" onClick={() => setView('apiKeys')} color="green" /><MenuItem title="助理工作室" icon="fa-user-astronaut" onClick={() => setView('prompts')} color="indigo" /><MenuItem title="模型管理" icon="fa-brain" onClick={() => setView('models')} color="purple" /></div> <div><h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">语音与交互</h4><div className="p-4 bg-gray-50 dark:bg-gray-700/50 rounded-2xl space-y-4"><div className="flex items-center justify-between"><label className="text-sm font-bold">语音合成引擎</label><select value={tempSettings.ttsEngine} onChange={(e) => handleChange('ttsEngine', e.target.value)} className="px-3 py-2 bg-white dark:bg-gray-600 rounded-lg text-sm border-0 outline-none"><option value={TTS_ENGINE.THIRD_PARTY}>Microsoft Edge (推荐)</option><option value={TTS_ENGINE.SYSTEM}>系统内置</option></select></div> {tempSettings.ttsEngine === TTS_ENGINE.THIRD_PARTY && (<div><label className="block text-sm font-bold mb-2">选择发音人</label><select value={tempSettings.ttsVoice} onChange={(e) => handleChange('ttsVoice', e.target.value)} className="w-full px-3 py-2 bg-white dark:bg-gray-600 rounded-lg text-sm border-0 outline-none">{MICROSOFT_TTS_VOICES.map(voice => <option key={voice.value} value={voice.value}>{voice.name}</option>)}</select></div>)} {tempSettings.ttsEngine === TTS_ENGINE.SYSTEM && (<div><label className="block text-sm font-bold mb-2">系统声音</label><select value={tempSettings.systemTtsVoiceURI} onChange={(e) => handleChange('systemTtsVoiceURI', e.target.value)} className="w-full px-3 py-2 bg-white dark:bg-gray-600 rounded-lg text-sm border-0 outline-none"><option value="">默认</option>{systemVoices.map(voice => <option key={voice.voiceURI} value={voice.voiceURI}>{`${voice.name} (${voice.lang})`}</option>)}</select></div>)} <div className="flex gap-4"><div className="flex-1"><div className="flex justify-between mb-1"><label className="text-xs font-bold">语速</label><span className="text-xs">{tempSettings.ttsRate}%</span></div><input type="range" min="-100" max="100" step="5" value={tempSettings.ttsRate} onChange={(e) => handleChange('ttsRate', parseInt(e.target.value, 10))} className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-blue-600"/></div><div className="flex-1"><div className="flex justify-between mb-1"><label className="text-xs font-bold">音调</label><span className="text-xs">{tempSettings.ttsPitch}%</span></div><input type="range" min="-100" max="100" step="5" value={tempSettings.ttsPitch} onChange={(e) => handleChange('ttsPitch', parseInt(e.target.value, 10))} className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-blue-600"/></div></div><div><label className="block text-sm font-bold mb-2">语音识别语言</label><select value={tempSettings.speechLanguage} onChange={(e) => handleChange('speechLanguage', e.target.value)} className="w-full px-3 py-2 bg-white dark:bg-gray-600 rounded-lg text-sm border-0 outline-none">{speechLanguageOptions.map(o => <option key={o.value} value={o.value}>{o.name}</option>)}</select></div><div className="flex items-center justify-between border-t border-gray-200 dark:border-gray-600 pt-3"><label className="text-sm font-bold">语音识别后自动发送</label><div className="relative inline-block w-12 mr-2 align-middle select-none"><input type="checkbox" checked={tempSettings.voiceAutoSend} onChange={(e) => handleChange('voiceAutoSend', e.target.checked)} className="toggle-checkbox absolute block w-6 h-6 rounded-full bg-white border-4 appearance-none cursor-pointer transition-transform duration-200 ease-in-out checked:translate-x-full checked:border-green-500"/><div className={`block overflow-hidden h-6 rounded-full cursor-pointer transition-colors ${tempSettings.voiceAutoSend ? 'bg-green-500' : 'bg-gray-300'}`}></div></div></div></div></div> <div><h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">个性化</h4><div className="p-4 bg-gray-50 dark:bg-gray-700/50 rounded-2xl space-y-4"><div><label className="block text-sm font-bold mb-2">用户头像</label><div className="flex gap-2 items-center"><img src={tempSettings.userAvatarUrl} alt="User" className="w-10 h-10 rounded-full object-cover border border-gray-200"/><input type="text" value={tempSettings.userAvatarUrl} onChange={(e) => handleChange('userAvatarUrl', e.target.value)} placeholder="头像 URL..." className="flex-1 px-3 py-2 bg-white dark:bg-gray-600 rounded-lg text-sm border-0 shadow-sm outline-none" /><button onClick={() => userAvatarInputRef.current?.click()} className="px-3 py-2 bg-gray-200 dark:bg-gray-600 rounded-lg font-bold text-sm"><i className="fas fa-upload"></i></button><input type="file" ref={userAvatarInputRef} onChange={handleUserAvatarSelect} accept="image/*" className="hidden" /></div></div><div><label className="block text-sm font-bold mb-2">聊天背景</label><div className="flex gap-2"><input type="text" value={tempSettings.chatBackgroundUrl} onChange={(e) => handleChange('chatBackgroundUrl', e.target.value)} placeholder="图片 URL..." className="flex-1 px-3 py-2 bg-white dark:bg-gray-600 rounded-lg text-sm border-0 shadow-sm outline-none" /><button onClick={() => fileInputRef.current?.click()} className="px-3 py-2 bg-gray-200 dark:bg-gray-600 rounded-lg font-bold text-sm"><i className="fas fa-image"></i></button><input type="file" ref={fileInputRef} onChange={handleBgImageSelect} accept="image/*" className="hidden" /></div></div><div className="flex items-center gap-3"><span className="text-sm font-bold">透明度</span><input type="range" min="0" max="100" value={tempSettings.backgroundOpacity} onChange={(e) => handleChange('backgroundOpacity', parseInt(e.target.value, 10))} className="flex-grow h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-blue-600" /><span className="text-sm w-8">{tempSettings.backgroundOpacity}%</span></div><div className="flex items-center justify-between"><label className="text-sm font-bold">每次打开开启新对话</label><div className="relative inline-block w-12 mr-2 align-middle select-none transition duration-200 ease-in"><input type="checkbox" name="toggle" id="toggle-newchat" checked={tempSettings.startWithNewChat} onChange={(e) => handleChange('startWithNewChat', e.target.checked)} className="toggle-checkbox absolute block w-6 h-6 rounded-full bg-white border-4 appearance-none cursor-pointer transition-transform duration-200 ease-in-out checked:translate-x-full checked:border-blue-500"/><label htmlFor="toggle-newchat" className={`toggle-label block overflow-hidden h-6 rounded-full cursor-pointer transition-colors ${tempSettings.startWithNewChat ? 'bg-blue-500' : 'bg-gray-300'}`}></label></div></div></div></div> <div className="border-t border-gray-200 dark:border-gray-600 pt-4"><div className="flex items-center justify-between"><label className="text-sm font-bold">自动朗读回复</label><div className="relative inline-block w-12 mr-2 align-middle select-none"><input type="checkbox" checked={tempSettings.autoRead} onChange={(e) => handleChange('autoRead', e.target.checked)} className="toggle-checkbox absolute block w-6 h-6 rounded-full bg-white border-4 appearance-none cursor-pointer transition-transform duration-200 ease-in-out checked:translate-x-full checked:border-green-500" /><label htmlFor="toggle-auto-read" className={`block overflow-hidden h-6 rounded-full cursor-pointer transition-colors ${tempSettings.autoRead ? 'bg-green-500' : 'bg-gray-300'}`}></label></div></div></div></div> <div className="p-4 border-t border-gray-100 dark:border-gray-700 bg-white/80 dark:bg-gray-800/80 backdrop-blur-md pb-safe"><button onClick={() => onSave(tempSettings)} className="w-full py-3.5 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-2xl shadow-lg shadow-blue-500/30 active:scale-95 transition-all">保存设置</button></div></div> )} {view === 'prompts' && <SubPageWrapper title="助理工作室" onBack={() => setView('main')} onSave={handleSubPageSave}><PromptManager prompts={tempSettings.prompts} settings={tempSettings} onChange={handlePromptSettingChange} onAdd={handleAddPrompt} onDelete={handleDeletePrompt} /></SubPageWrapper>} {view === 'models' && <SubPageWrapper title="模型管理" onBack={() => setView('main')} onSave={handleSubPageSave}><ModelManager models={tempSettings.chatModels} onChange={handleModelSettingChange} onAdd={handleAddModel} onDelete={handleDeleteModel} /></SubPageWrapper>} {view === 'apiKeys' && <SubPageWrapper title="API 密钥管理" onBack={() => setView('main')} onSave={handleSubPageSave}><ApiKeyManager apiKeys={tempSettings.apiKeys} activeApiKeyId={tempSettings.activeApiKeyId} onChange={handleApiKeySettingChange} onAdd={handleAddApiKey} onDelete={handleDeleteApiKey} onSetActive={handleSetActiveApiKey} /></SubPageWrapper>} </div> </div> ); };
...
const AiChatAssistant = ({ onClose }) => {
    ...
    useEffect(() => { setIsMounted(true); let finalSettings = { ...DEFAULT_SETTINGS }; const savedSettings = safeLocalStorageGet('ai_chat_settings'); if (savedSettings) { const parsed = JSON.parse(savedSettings); if (parsed.thirdPartyTtsConfig) { parsed.ttsVoice = parsed.thirdPartyTtsConfig.microsoftVoice || DEFAULT_SETTINGS.ttsVoice; delete parsed.thirdPartyTtsConfig; } parsed.prompts = (parsed.prompts || []).map(p => ({ ...p, model: p.model || DEFAULT_SETTINGS.selectedModel, ttsVoice: p.ttsVoice || DEFAULT_SETTINGS.ttsVoice, avatarUrl: p.avatarUrl || '', description: p.description || '' })); if (!parsed.chatModels || parsed.chatModels.length === 0) { parsed.chatModels = CHAT_MODELS_LIST; } if (!parsed.apiKeys) { parsed.apiKeys = []; } finalSettings = { ...DEFAULT_SETTINGS, ...parsed }; } if (typeof navigator !== 'undefined' && /FBAN|FBAV/i.test(navigator.userAgent)) { finalSettings.isFacebookApp = true; } setSettings(finalSettings); const savedConversations = safeLocalStorageGet('ai_chat_conversations'); const parsedConvs = savedConversations ? JSON.parse(savedConversations) : []; setConversations(parsedConvs); if (finalSettings.startWithNewChat || parsedConvs.length === 0) { createNewConversation(finalSettings.currentPromptId, true); } else { const firstConv = parsedConvs; setCurrentConversationId(firstConv.id); } 
        // 关键修复：组件卸载时清理所有音频
        return () => { window.speechSynthesis.cancel(); globalAudioQueue.clear(); }
    }, []);
    ...
    const fetchAiResponse = async (messagesForApi) => {
        ...
        try {
            ...
            const modelInfo = (settings.chatModels || []).find(m => m.value === settings.selectedModel) || CHAT_MODELS_LIST;
            ...
            let aiResponseContent = activeKey.provider === 'gemini' ? data.candidates?.?.content?.parts?.?.text : data.choices?.?.message?.content;
            if (!aiResponseContent) throw new Error('AI未能返回有效内容。');
            
            let aiMessage;
            try { const jsonMatch = aiResponseContent.match(/\{[\s\S]*\}/); if (jsonMatch) { const parsed = JSON.parse(jsonMatch); if (parsed && parsed.component && parsed.props && componentMap[parsed.component]) { aiMessage = { role: 'ai', content: null, timestamp: Date.now(), isComponent: true, componentName: parsed.component, props: sanitizeQuizData(parsed.props), isTyping: false }; } else { throw new Error(); } } else { throw new Error(); } } catch (e) { aiMessage = { role: 'ai', content: aiResponseContent, timestamp: Date.now(), isTyping: true }; }
            setConversations(prev => prev.map(c => c.id === currentConversationId ? { ...c, messages: [...messagesForApi, aiMessage] } : c));
        } catch (err) {
            let errorMessage = err.name === 'AbortError' ? '请求被中断' : `请求错误: ${err.message}`;
            setError(errorMessage);
            setConversations(prev => prev.map(c => c.id === currentConversationId ? { ...c, messages: [...messagesForApi, { role: 'ai', content: `抱歉，出错了: ${errorMessage}`, timestamp: Date.now() }] } : c));
        } finally { setIsLoading(false); }
    };
    
    const handleSubmit = async (isRegenerate = false, textToSend = null) => { if (!currentConversation) return; let messagesForApi = [...currentConversation.messages]; if (isRegenerate) { if (messagesForApi.length > 0 && messagesForApi[messagesForApi.length - 1].role === 'ai') { messagesForApi.pop(); } } else { const textToProcess = (textToSend !== null ? textToSend : userInput).trim(); if (!textToProcess && selectedImages.length === 0) { setError('请输入文字或添加图片后再发送！'); return; } const userMessage = { role: 'user', content: textToProcess, images: selectedImages, timestamp: Date.now() }; messagesForApi.push(userMessage); setConversations(prev => prev.map(c => c.id === currentConversationId ? { ...c, messages: messagesForApi } : c)); setUserInput(''); setSelectedImages([]); } await fetchAiResponse(messagesForApi); };
            ...
            <div className="relative flex flex-1 min-h-0">
                ...
                <div className="flex-1 flex flex-col h-full min-w-0">
                    <header className="flex items-center justify-between py-2 px-2 shrink-0 bg-white/40 dark:bg-black/20 backdrop-blur-lg shadow-sm border-b border-gray-200/50 dark:border-gray-800/50">
                        <div className="flex items-center gap-2"><button onClick={() => setIsSidebarOpen(s => !s)} className="p-2 rounded-full text-gray-600 dark:text-gray-300 hover:bg-black/10 dark:hover:bg-white/10 active:scale-95" title="对话列表"><i className="fas fa-bars text-xl"></i></button></div>
                    ...
                    <main className="flex-grow p-4 overflow-y-auto">
                        <div className="space-y-1">
                            {currentConversation?.messages.map((msg, index) => ( <div id={`msg-${currentConversation.id}-${index}`} key={`${currentConversation.id}-${index}`}> <MessageBubble msg={msg} settings={settings} isLastAiMessage={index === currentConversation.messages.length - 1 && msg.role === 'ai'} onRegenerate={() => handleSubmit(true)} onTypingComplete={handleTypingComplete} onTypingUpdate={scrollToBottom} onCorrectionRequest={()=>{}} explicitAiAvatar={displayAiAvatar} onTranslate={handleTranslate} /> </div> ))}
