...
const audioController = {
  ...
  async preload(text) {
    ...
    try {
      ...
      const blob = await res.blob();
      await idb.set(cacheKey, blob);
      console.log('Preloaded audio for:', textToRead);
    } catch (e) {
...
/**
 * 提前缓存下一课所需的所有音频
 * @param {Array<string>} texts - 包含下一课所有需要朗读的文本（题目、选项、解析等）的数组
 */
export const preloadNextLessonAudios = (texts = []) => {
    if (!Array.isArray(texts)) return;
    console.log("Starting preload for next lesson...");
    // 使用Promise.allSettled确保即使部分失败，其他也能继续
...
const generatePinyinData = (text) => {
  if (!text) return [];
  try {
    const pinyins = pinyin(text, { type: 'array', toneType: 'none' }) || []; // toneType: 'none'
...
const XuanZeTi = ({ question = {}, options = [], correctAnswer = [], onCorrect, explanation }) => {
  ...
  const handleSelect = (option) => {
    ...
    if (option.text) audioController.play(option.text, 0.9);
  };
  ...
  const handleSubmit = () => {
    ...
    if (isCorrect) {
      ...
      // 允许重选
      setTimeout(() => {
        setIsSubmitted(false); // 解锁交互，允许用户重新选择或再次提交
      }, 1500);
    }
  };
