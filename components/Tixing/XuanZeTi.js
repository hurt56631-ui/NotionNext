...
const useMixedTTS = () => {
    ...
    const splitMixedText = (text) => {
        ...
        while ((match = regex.exec(text)) !== null) {
            ...
            if (myanmarText && myanmarText.trim()) segments.push({ lang: 'mm', text: myanmarText.trim() });
            else if (chineseText && chineseText.trim()) segments.push({ lang: 'zh', text: chineseText.trim() });
            else if (otherText && segments.length > 0) segments[segments.length - 1].text += otherText;
        }
...
const cssStyles = `
  ...
  .xzt-question-card { background: #ffffff; border-radius: 20px; padding: 24px 20px; text-align: center; box-shadow: 0 4px 20px rgba(0,0,0,0.05); border: 1px solid #f1f5f9; cursor: pointer; width: 100%; max-width: 460px; margin: 0 auto 10px auto; display: flex; flex-direction: column; align-items: center; position: relative; user-select: none; }
  ...
  .title-mixed-box { display: flex; flex-wrap: wrap; justify-content: center; gap: 4px; row-gap: 8px; align-items: flex-end; margin-top: 4px; line-height: 1.4; }
  ...
  .fixed-bottom-area { position: fixed; bottom: 80px; left: 0; right: 0; display: flex; flex-direction: column; align-items: center; z-index: 100; pointer-events: none; padding: 0 20px; }
  ...
  .submit-btn { pointer-events: auto; width: auto; min-width: 200px; max-width: 80%; padding: 12px 40px; border-radius: 999px; font-size: 1.1rem; font-weight: 800; letter-spacing: 0.5px; color: white; background: linear-gradient(135deg, #8b5cf6 0%, #6366f1 100%); box-shadow: 0 8px 20px rgba(99, 102, 241, 0.3); border: none; transition: all 0.2s; text-align: center; cursor: pointer; }
        ...
        <div className="xzt-options-grid">
          {displayOptions.map(option => {
            ...
            return (
              ...
              >
                {option.hasImage && <div className="opt-img-wrapper"><img src={option.imageUrl} alt="opt" className="opt-img" /></div>}
                <div className="opt-text-box"><MixedLanguageText text={option.text} /></div>
