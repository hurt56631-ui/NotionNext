...
// =================================================================================
const DB_NAME = 'MixedAudioSegmentCache'; // 使用新的DB Name避免冲突
...
const useMixedTTS = () => {
    ...
    const fetchSegmentAudio = async (text, lang) => {
        ...
        if (!res.ok) throw new Error(`TTS Fetch Failed for ${lang}: ${res.status}`);
...
const cssStyles = `
  ...
  .xzt-container { width: 100%; height: 100vh; display: flex; flex-direction: column; align-items: center; position: relative; padding: 10px 32px; box-sizing: border-box; overflow-y: auto; -webkit-overflow-scrolling: touch; -webkit-tap-highlight-color: transparent; }
  .spacer { flex: 1; min-height: 10px; max-height: 40px; }
  .xzt-question-card { background: #ffffff; border-radius: 20px; padding: 20px; text-align: center; box-shadow: 0 4px 20px rgba(0,0,0,0.05); border: 1px solid #f1f5f9; cursor: pointer; width: 100%; max-width: 460px; margin: 0 auto 10px auto; display: flex; flex-direction: column; align-items: center; position: relative; user-select: none; }
  ...
  .pinyin-box { display: flex; flex-wrap: wrap; justify-content: center; gap: 4px; row-gap: 8px; align-items: flex-end; margin-top: 4px; }
  ...
  .xzt-option-card { position: relative; background: #ffffff; border-radius: 16px; border: 2px solid #e2e8f0; box-shadow: 0 2px 5px rgba(0, 0, 0, 0.03); cursor: pointer; transition: all 0.2s cubic-bezier(0.25, 0.8, 0.25, 1); user-select: none; display: flex; align-items: center; width: 100%; box-sizing: border-box; min-height: auto; }
  ...
  .fixed-bottom-area { position: fixed; bottom: 100px; left: 32px; right: 32px; display: flex; flex-direction: column; align-items: center; z-index: 100; pointer-events: none; }
  .explanation-card { pointer-events: auto; background: #fff; border: 1px solid #fecaca; background-color: #fff1f2; color: #991b1b; padding: 12px 14px; border-radius: 12px; margin-bottom: 12px; font-size: 0.9rem; line-height: 1.4; text-align: left; width: 100%; max-width: 460px; box-shadow: 0 4px 15px rgba(239, 68, 68, 0.15); display: flex; gap: 10px; align-items: flex-start; animation: slideUp 0.3s ease-out; }
  .submit-btn { pointer-events: auto; width: 100%; max-width: 460px; padding: 14px 0; border-radius: 999px; font-size: 1.1rem; font-weight: 800; letter-spacing: 0.5px; color: white; background: linear-gradient(135deg, #8b5cf6 0%, #6366f1 100%); box-shadow: 0 8px 20px rgba(99, 102, 241, 0.3); border: none; transition: all 0.2s; text-align: center; cursor: pointer; }
  .submit-btn:disabled { background: #e2e8f0; color: #cbd5e1; box-shadow: none; }
  ...
  try {
    ...
    return chars.map((char) => {
      if (isChineseChar(char)) {
        let py = pinyins[pyIndex] || ''; pyIndex++; return { char, pinyin: py };
      } 
      return { char, pinyin: '' };
    });
  } catch (e) {
...
const XuanZeTi = ({ 
  ...
  useEffect(() => {
    ...
    const chinesePart = text.match(/[\u4e00-\u9fa5]+/g)?.join('') || "";
    setQuestionPinyin(generatePinyinData(chinesePart));
    ...
    setTimeout(() => {
      if (isMounted.current && text) {
        play(text, `question_${question.id || currentIndex}`);
      }
    }, 500);
  ...
  const MixedLanguageText = ({ text }) => {
    if (!text) return null;
    const parts = text.split(/([()])/).filter(Boolean); // 按括号分割
    return (
      <>
      ...
      <div className="xzt-container">
        ...
        <div className="fixed-bottom-area">
          ...
          <button className="submit-btn" onClick={handleSubmit} disabled={!selectedId || (isSubmitted && !showExplanation)}>
            {isSubmitted ? (getCorrectIds(question, correctAnswer || question.correctId).includes(String(selectedId)) ? "正确" : "请重试") : "确认"}
          </button>
        </div>
