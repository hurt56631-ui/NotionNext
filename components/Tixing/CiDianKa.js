// components/Tixing/CiDianKa.js (V30 - TikTok Layout & Auto-Switch)
...
const styles = {
  ...
  cardInner: { position: 'relative', width: '100%', height: '100%', transformStyle: 'preserve-3d', transition: 'transform 0.8s cubic-bezier(0.6, 0, 0.4, 1)' },
  ...
  mainContent: { flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', position: 'relative', overflowY: 'auto', cursor: 'pointer', padding: '10px', zIndex: 1 },
...
const useCardSettings = () => {
  const [settings, setSettings] = useState(() => {
    try {
      ...
      return { order: 'sequential', autoPlayFront: true, autoPlayBack: true, autoSwitchNext: true, frontFontSize: '6rem', frontColor: '#ffffff', frontShadow: '1px 1px 3px rgba(0,0,0,0.9), 0 0 10px rgba(0,0,0,0.7)', backFontSize: '1.2rem', backColor: '#2d3748', backShadow: 'none' }; 
    }
  });
  useEffect(() => { 
    try { 
...
const DetailedPronunciationChecker = ({ correctWord, userText, onCorrect }) => {
  ...
  return (<div style={styles.pronunciationChecker} onClick={(e)=>e.stopPropagation()} data-no-flip="true"><h3 style={{ fontWeight: 'bold', fontSize: '1.25rem', color: '#1a202c', marginTop: 0 }}>发音分析</h3>{result.error?(<p style={{ marginTop: 8, color: '#dc2626' }}>{result.error}</p>):(<div style={{ marginTop: 12, display: 'flex', flexDirection: 'column', gap: 12 }}><div><span style={{ fontWeight: 600, width: 90, color: '#4a5568', display: 'inline-block' }}>标准发音:</span><span style={{ fontFamily: 'monospace', fontSize: '1.2rem', fontWeight: 'bold', color: '#16a34a', wordBreak: 'break-all' }}>{result.correctRaw}</span></div><div><span style={{ fontWeight: 600, width: 90, color: '#4a5568', display: 'inline-block' }}>你的发音:</span><span style={{ fontFamily: 'monospace', fontSize: '1.2rem', fontWeight: 'bold', wordBreak: 'break-all' }}>{result.details.map((d, idx) => { const iColor = d.flags.initialCorrect ? '#1a202c' : '#dc2626'; const fColor = d.flags.finalCorrect ? '#1a202c' : '#dc2626'; const tColor = d.flags.toneCorrect ? '#1a202c' : '#dc2626'; return (<span key={idx} style={{ display: 'inline-block', marginRight: 10 }}><span style={{ color: iColor }}>{d.user.initial}</span><span style={{ color: fColor }}>{d.user.final}</span><sup style={{ marginLeft: 4, color: tColor }}>{d.user.tone ?? ''}</sup></span>); })}</span></div>{!result.overallCorrect && (<p style={{ paddingTop: 8, color: '#ca8a04', borderTop: '1px solid #e2e8f0', marginTop: 8 }}>提示：红色为不匹配项，请重试。</p>)}</div>)}</div>);
};
...
const CiDianKa = ({ flashcards = [] }) => {
  ...
  const bind = useDrag(({ down, movement: [, my], direction: [, yDir], velocity: [, vy], tap, event }) => {
    if (tap) {
    ...
    const trigger = vy > 0.3 && Math.abs(my) > 80;
    if (!down && trigger) {
  ...
  }, [currentIndex, isFlipped, settings.autoSwitchNext, navigate]);
  ...
  return (
    <div style={styles.fullScreen}>
      ...
      {transitions((style, i, t, item) => {
        const cardData = cards[i];
        if (!cardData) return null;
        const backgroundStyle = getBgStyle(currentBg);
        return (
          <animated.div key={i} style={{ ...styles.animatedCardShell, ...style }}>
            <div style={styles.cardContainer} {...bind()} onClickCapture={e => recognizedText && setRecognizedText('')}>
      ...
      {/* 右侧控制按钮 */}
      <div style={styles.rightControls} data-no-flip="true">
        ...
        <button style={styles.rightIconButton} onClick={() => setWriterChar(cards[currentIndex]?.word)} title="笔顺"><FaPenFancy size={24} /></button>
        <button style={styles.rightIconButton} onClick={(e) => playTTS(cards[currentIndex]?.word, null, e)} title="朗读"><FaVolumeUp size={24} /></button>
      </div>
    </div>
  );
};
