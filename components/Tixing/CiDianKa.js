// components/Tixing/CiDianKa.js (V16 - 修复 & 优化版)
...
const styles = {
  fullScreen: {
    position: 'fixed', inset: 0, zIndex: 9999, background: '#e9eef3',
    display: 'flex', alignItems: 'center', justifyContent: 'center',
    overflow: 'hidden', touchAction: 'none'
  },
  container: { position: 'relative', width: '100%', height: '100%' },
  deck: {
    position: 'absolute', width: '92%', maxWidth: '900px',
    height: '86%', maxHeight: '720px',
    display: 'flex', alignItems: 'center', justifyContent: 'center'
  },
  cardInner: {
  ...
  face: {
    ...
    borderRadius: '20px', color: '#1a202c',
    boxShadow: '0 30px 60px rgba(0,0,0,0.12)',
    display: 'flex', flexDirection: 'column',
    padding: '28px',
  },
  backFace: { transform: 'rotateY(180deg)', background: '#ffffff' },
  mainContent: { flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' },
  header: { textAlign: 'center' },
  pinyin: { fontSize: '1.4rem', color: '#4a5568', marginBottom: 6 },
  hanzi: { fontSize: '5.6rem', fontWeight: 800, lineHeight: 1.05 },
  footer: {
    display: 'flex', justifyContent: 'space-between', alignItems: 'center',
    gap: 12, marginTop: 'auto', borderTop: '1px solid rgba(0,0,0,0.08)',
    paddingTop: 12
  },
  button: {
    background: 'rgba(255, 255, 255, 0.6)', color: '#2d3748',
  ...
  feedbackMessage: { color: '#4b5563', minHeight: '24px', fontSize: '1.1rem', display: 'flex', alignItems: 'center', gap: 10 },
  ...
  example: { background: 'rgba(240,244,255,0.7)', padding: '12px 16px', borderRadius: 12, marginTop: 20, display: 'flex', gap: 10, alignItems: 'center' },
...
const CiDianKa = ({ flashcards = [] }) => {
  const cards = flashcards.length ? flashcards : [
    { word: "示例", pinyin: "shì lì", meaning: "Example", example: "这是一个示例。" }
  ];
  ...
  const handleListen = useCallback((e) => {
    ...
    recognition.onresult = (event) => {
      ...
      setSpeechResult({
        msg: pinyinFeedback.every(p => p.status === 'correct') ? '完全正确！' : '请看对比结果',
        pinyin: pinyinFeedback,
        transcript
      });
    };
                    ...
                    <div style={styles.feedbackArea}>
                      ...
                      <div style={styles.feedbackPinyinRow}>
                        {speechResult.pinyin.map((syl, idx) => (
                          <div key={idx} style={{
                            ...styles.feedbackPinyinSyllable,
                            background: syl.status === 'correct' ? '#dcfce7' : '#fee2e2',
                            color: syl.status === 'correct' ? '#166534' : '#991b1b'
                          }}>
                            <span>{syl.correct}</span>
                            <span style={{ fontSize: '0.9rem', opacity: 0.7 }}>{syl.spoken}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                ...
                <div style={{ ...styles.face, ...styles.backFace }}>
                  <div style={styles.mainContent}>
                    ...
                    <div style={styles.meaning}>
                      {cards[i].meaning}
                      <FaVolumeUp style={{ cursor: 'pointer' }} onClick={(e) => { e.stopPropagation(); playTTS(cards[i].meaning); }} />
                    </div>
                    {cards[i].example && (
                      <div style={styles.example}>
                        <FaVolumeUp style={{ cursor: 'pointer' }} onClick={(e) => { e.stopPropagation(); playTTS(cards[i].example); }} />
                        {cards[i].example}
                      </div>
                    )}
                  </div>
