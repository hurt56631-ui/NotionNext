// components/Tixing/CiDianKa.js (修复编译错误，优化图片加载、预加载、样式调整)
...
const PinyinSeparatedText = React.memo(({ text }) => {
    const pinyinData = useMemo(() => {
        try { if (typeof text !== 'string' || !text || !/[\u4e00-\u9fa5]/.test(text)) { return { pinyin: '', hanzi: text }; } const pinyinString = pinyinConverter(text, { toneType: 'mark', separator: ' ' }); return { pinyin: pinyinString, hanzi: text }; } catch (error) { console.error("PinyinSeparatedText Error:", error, { text }); return { pinyin: '', hanzi: text }; }
    }, [text]);
...
// 优化：移除 useTransition，使用基础 opacity 切换，强制 30% 压缩
