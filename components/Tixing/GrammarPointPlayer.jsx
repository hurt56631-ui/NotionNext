// components/Tixing/GrammarPointPlayer.jsx (已修改为内容全屏)
                                ...
                                <div style={styles.examplesList}>
                                    {gp.examples.map((ex, index) => (
                                        <div key={ex.id} style={styles.exampleItem}>
                                            ...
                                            <button className="play-button" style={styles.playButton} onClick={() => handlePlayButtonClick(ex.narrationScript || ex.sentence, `example_${ex.id}`)}>
...
GrammarPointPlayer.propTypes = {
    grammarPoints: PropTypes.arrayOf(PropTypes.shape({
        ...
        chineseVoice: PropTypes.string,
        myanmarVoice: PropTypes.string,
...
const styles = {
    ...
    contentWrapper: { 
        width: '100%', 
        // maxWidth: '500px',  // <-- 我注释掉了这一行
        maxWidth: '1200px', //  <-- 或者您可以设置一个更大的宽度，比如1200px，以适应宽屏
        margin: '0 auto', //     <-- 增加这一行让它在超宽屏上也能居中
        height: '100%', 
        display: 'flex', 
