...
const playPinyinR2Audio = (filename) => {
    return new Promise((resolve) => {
        if (isStoppingRef.current) {
            resolve();
            return;
        }
        stopAllAudio(); // Stop other sounds before playing the new one

        const cleanFilename = filename.trim().replace('.mp3', '');
        ...
        const sound = new Howl({
            ...
            onend: resolve,
            onloaderror: (id, err) => {
                console.warn(`❌ 缺文件或加载失败: ${audioSrc}`, err);
                resolve(); // Continue sequence even if a file is missing
            },
            onplayerror: (id, err) => {
                console.error(`播放R2拼读音频失败: ${audioSrc}`, err);
                resolve(); // Continue sequence on play error
            }
        });
        
        _howlInstance = sound; // Track the current instance for global stop
        sound.play();
    });
};
...
const startSpelling = async () => {
    ...
    for (let i = 0; i < chars.length; i++) {
        ...
        if (isWhole) {
            setStatus(`${i}-full`);
            await playPinyinR2Audio(`${pinyinWithTone}.mp3`);
            await new Promise(r => setTimeout(r, 400));
        } else if (pData.initial) {
            setStatus(`${i}-initial`);
            await playPinyinR2Audio(`${pData.initial}.mp3`);
            await new Promise(r => setTimeout(r, 150));
            if (isStoppingRef.current) break;

            setStatus(`${i}-final`);
            const finalFile = getCorrectFinalFilename(pData);
            await playPinyinR2Audio(finalFile);
            await new Promise(r => setTimeout(r, 150));
            if (isStoppingRef.current) break;
            ...
            setStatus(`${i}-full`);
            await playPinyinR2Audio(`${pData.final}.mp3`);
            await new Promise(r => setTimeout(r, 400));
        }
    }
