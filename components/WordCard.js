// components/WordCard.js (基于能正常工作的 ShortSentenceCard 重构)
...
// =================================================================================
// ===== IndexedDB & 辅助工具 (已适配为单词) =====================================
// =================================================================================
const DB_NAME = 'ChineseLearningDB';
const STORE_NAME = 'favoriteWords'; // 存储名称改为单词
...
const PronunciationComparison = React.memo(({ correctWord, userText, userAudioUrl, onContinue, onClose, onPlayStandard }) => {
    const analysis = useMemo(() => {
        ...
        const results = correctPinyin.map((correctPy, index) => ({ char: correctWord[index], isMatch: correctPy === (userPinyin[index] || ''), correctPinyin: correctPy, userPinyin: userPinyin[index] || '' }));
...
const WordCard = ({ words = [], isOpen, onClose, progressKey = 'default' }) => {
  ...
  const cards = useMemo(() => {
    // 适配：数据源是 words, 字段是 chinese, burmese
  ...
  const handlePronunciationPractice = useCallback(async (e) => {
      e.stopPropagation();
      if (_howlInstance?.playing()) _howlInstance.stop();
      if (isRecording) { mediaRecorderRef.current?.stop(); recognitionRef.current?.stop(); playSoundEffect('recordStop'); return; }
      const SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition;
      if (!SpeechRecognition || !navigator.mediaDevices?.getUserMedia) { alert('抱歉，您的浏览器不支持录音或语音识别功能。'); return; }
      try {
          const stream = await navigator.mediaDevices.getUserMedia({ audio: true });
          playSoundEffect('recordStart'); setIsRecording(true);
          ...
          recognition.onend = () => { if (mediaRecorderRef.current?.state === 'recording') mediaRecorderRef.current.stop(); recognitionRef.current = null; };
          const recorder = new MediaRecorder(stream);
          audioChunksRef.current = [];
          recorder.ondataavailable = (event) => audioChunksRef.current.push(event.data);
          recorder.onstop = () => { const audioBlob = new Blob(audioChunksRef.current, { type: 'audio/webm' }); setUserAudioUrl(URL.createObjectURL(audioBlob)); stream.getTracks().forEach(track => track.stop()); setIsRecording(false); };
          recognitionRef.current = recognition; mediaRecorderRef.current = recorder;
          recognition.start(); recorder.start();
...
export default WordCard; // 导出正确的组件名
