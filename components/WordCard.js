...
const playTTS = async (text, voice, rate, onEndCallback, e) => {
    ...
    try {
        ...
        _howlInstance = new Howl({
            ...
            onloaderror: (id, err) => {
                console.error('Howler 加载音频错误:', err);
                URL.revokeObjectURL(audioUrl);
                if (onEndCallback) onEndCallback();
            },
            onplayerror: (id, err) => {
                console.error('Howler 播放音频错误:', err);
                URL.revokeObjectURL(audioUrl);
                if (onEndCallback) onEndCallback();
            }
        });
  ...
  const cardContent = pageTransitions((style, item) =>
    item && (
      <animated.div style={{ ...styles.fullScreen, ...style }}>
        ...
        {/* [MODIFIED] Controls are now centered at the bottom */}
        {currentCard && (
            <div style={styles.bottomControls} data-no-gesture="true">
                <button style={styles.controlButton} onClick={() => setIsSettingsOpen(true)} title="设置">
                    <FaCog size={20} />
                </button>
                <button style={styles.controlButton} onClick={handleListen} title="发音练习">
                    <FaMicrophone size={20} color={isListening ? '#dc2626' : '#4a5568'} />
                </button>
                
                {currentCard.chinese && currentCard.chinese.length > 0 && 
                 currentCard.chinese.length <= 5 && 
                 !currentCard.chinese.includes(' ') && ( 
                    <button style={styles.controlButton} onClick={() => setWriterChar(currentCard.chinese)} title="笔顺">
                        <FaPenFancy size={20} />
                    </button>
                )}
...
const styles = {
    ...
    // [MODIFIED] New style for centered bottom controls
    bottomControls: { 
        position: 'fixed', 
        bottom: '80px', 
        left: '50%', 
    ...
    // [MODIFIED] A single style for all control buttons
    controlButton: { 
        background: 'white', 
        border: 'none', 
        cursor: 'pointer', 
    ...
    // [REMOVED] rightControls and exitButtonInControls are no longer needed
