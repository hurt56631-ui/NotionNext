// components/WordCard.js (最终修复版：支持 audioText 多音字朗读 + 识别与录音分离)
...
const PronunciationComparison = ({ correctWord, audioText, userText, settings, onContinue, onClose }) => {
    const analysis = useMemo(() => {
        if (!userText) { return { isCorrect: false, error: 'NO_PINYIN', message: '未能识别有效发音' }; }
        
        // Use the displayed word for pinyin comparison (what user sees)
        ...
        const maxLength = Math.max(correctPinyin.length, userPinyin.length);

        for (let i = 0; i < maxLength; i++) {
            const correctSyllable = correctPinyin[i] || '';
            const userSyllable = userPinyin[i] || '';
            const isMatch = correctSyllable === userSyllable;
            if (isMatch && correctSyllable) {
                correctCount++;
            }
            comparisonResult.push({ text: userSyllable, isCorrect: isMatch });
        }
    ...
    // ✅ Play TTS using audioText if available (for polyphones), otherwise correctWord
    const playCorrectTTS = useCallback(() => { 
        const textToPlay = audioText || correctWord;
        playTTS(textToPlay, settings.voiceChinese, settings.speechRateChinese); 
    }, [correctWord, audioText, settings]);
            ...
            <div style={styles.comparisonPanel}>
                ...
                <div style={styles.errorDetailsContainer}>
                    {analysis.error ? (
                        ...
                        <div style={styles.pinyinComparisonBox}>
                            <div style={styles.pinyinComparisonRow}>
                                <div style={styles.pinyinLabel}>标准发音</div>
                                <div style={styles.pinyinText}>{analysis.correctPinyinStr}</div>
                            </div>
                            <div style={styles.pinyinComparisonRow}>
...
const WordCard = ({ words = [], isOpen, onClose, progressKey = 'default' }) => {
  ...
  // ✅ [MODIFIED] Process cards to include audioText support
  const processedCards = useMemo(() => {
    try {
        const mapped = words.map(w => ({
            id: w.id,
            chinese: w.chinese,
            // ✅ Read 'audioText' if it exists in data, otherwise use 'chinese'
            audioText: w.audioText || w.chinese,
            burmese: w.burmese,
            mnemonic: w.mnemonic,
            example: w.example,
        }));
  ...
  useEffect(() => {
    const initialCards = processedCards.length > 0 ? processedCards : [{ id: 'fallback', chinese: "暂无单词", audioText: "暂无单词", burmese: "..." }];
  ...
  useEffect(() => {
    ...
    const playFullSequence = () => {
        if (settings.autoPlayChinese && currentCard.chinese) {
            // ✅ [MODIFIED] Use audioText for TTS playback to support polyphones
            playTTS(currentCard.audioText, settings.voiceChinese, settings.speechRateChinese, () => {
  ...
  const cardContent = pageTransitions((style, item) => {
    const backgroundStyle = settings.backgroundImage
        ? { background: `url(${settings.backgroundImage}) center/cover no-repeat`, backgroundAttachment: 'fixed' }
        : {};
      ...
      <animated.div style={{ ...styles.fullScreen, ...style, ...backgroundStyle }}>
        ...
        {isComparisonOpen && currentCard && (
            <PronunciationComparison
                ...
                audioText={currentCard.audioText}
        ...
        {activeCards.length > 0 && currentCard ? (
                ...
                <animated.div key={cardData.id} style={{ ...styles.animatedCardShell, ...cardStyle }}>
                  <div style={styles.cardContainer}>
                      <div style={{ textAlign: 'center' }}>
                          <div style={{ cursor: 'pointer' }} onClick={(e) => playTTS(cardData.audioText, settings.voiceChinese, settings.speechRateChinese, null, e)}><div style={styles.pinyin}>{pinyinConverter(cardData.chinese, { toneType: 'symbol', separator: ' ' })}</div><div style={styles.textWordChinese}>{cardData.chinese}</div></div>
                          {isRevealed && (
            ...
            <div style={styles.completionContainer}>
                <h2>🎉 全部完成！</h2>
                <p>你已学完本列表中的所有单词。</p>
                <button style={styles.knowButton} onClick={handleCloseRequest}>关闭</button>
            </div>
        )}
...
// =================================================================================
const styles = {
    fullScreen: { position: 'fixed', inset: 0, zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', touchAction: 'none', backgroundColor: '#53818E' },
