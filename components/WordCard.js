// components/WordCard.js (最终方案 - Web Audio API 旁路录音)
...
const PronunciationComparison = ({ correctWord, userText, userAudioURL, settings, onContinue, onClose }) => { const hasRecognitionResult = userText && userText.length > 0; const analysis = useMemo(() => { if (!hasRecognitionResult) return null; const correctPinyin = pinyinConverter(correctWord, { toneType: 'num', type: 'array', removeNonHan: true }); const userPinyin = pinyinConverter(userText, { toneType: 'num', type: 'array', removeNonHan: true }); if (correctPinyin.length === 0 || userPinyin.length === 0) return { isCorrect: false, error: 'NO_PINYIN', message: '无法识别有效发音' }; if (correctPinyin.length !== userPinyin.length) return { isCorrect: false, error: 'LENGTH_MISMATCH', message: `字数不对：应为 ${correctPinyin.length} 字，你读了 ${userPinyin.length} 字` }; const results = correctPinyin.map((correctPy, index) => { const userPy = userPinyin[index]; const correctParts = parsePinyin(correctPy); const userParts = parsePinyin(userPy); const errors = { initial: (correctParts.initial || userParts.initial) && (correctParts.initial !== userParts.initial), final: correctParts.final !== userParts.final, tone: correctParts.tone !== userParts.tone, }; const pinyinMatch = !errors.initial && !errors.final && !errors.tone; return { char: correctWord[index], pinyinMatch, correct: { parts: correctParts }, user: { parts: userParts, errors } }; }); const isCorrect = results.every(r => r.pinyinMatch); const accuracy = (results.filter(r => r.pinyinMatch).length / results.length * 100).toFixed(0); return { isCorrect, results, accuracy }; }, [correctWord, userText, hasRecognitionResult]); useEffect(() => { if (analysis) { playSoundEffect(analysis.isCorrect ? 'correct' : 'incorrect'); } }, [analysis]); const playUserAudio = useCallback(() => { if (userAudioURL) { if (_howlInstance?.playing()) _howlInstance.stop(); const sound = new Howl({ src: [userAudioURL], html5: true }); sound.play(); } }, [userAudioURL]); const playCorrectTTS = useCallback(() => { playTTS(correctWord, settings.voiceChinese, settings.speechRateChinese); }, [correctWord, settings]); const headerContent = useMemo(() => { if (!hasRecognitionResult) return { icon: '🤔', title: '未能识别语音', subtitle: '请听录音自行对比', gradient: 'linear-gradient(135deg, #6b7280, #4b5563)' }; if (analysis.isCorrect) return { icon: '🎉', title: '发音完美！', subtitle: '太棒了！', gradient: 'linear-gradient(135deg, #10b981, #059669)' }; return { icon: '💪', title: `准确率: ${analysis.accuracy}%`, subtitle: '再接再厉！', gradient: 'linear-gradient(135deg, #ef4444, #dc2626)' }; }, [analysis, hasRecognitionResult]); return ( <div style={styles.comparisonOverlay}> <div style={styles.comparisonPanel}> <div style={{...styles.resultHeader, background: headerContent.gradient}}> <div style={{ fontSize: '2.5rem' }}>{headerContent.icon}</div> <div style={{ fontSize: '1.5rem', fontWeight: 'bold' }}>{headerContent.title}</div> <div style={{ fontSize: '1rem', marginTop: '8px' }}>{headerContent.subtitle}</div> </div> <div style={styles.errorDetailsContainer}> {hasRecognitionResult && analysis ? ( analysis.error ? ( <div style={styles.lengthError}><h3>{analysis.message}</h3></div> ) : ( <div style={styles.comparisonGrid}> {analysis.results.map((result, index) => ( <div key={index} style={styles.comparisonCell}> <div style={styles.comparisonChar}>{result.char}</div> <div style={styles.comparisonPinyinGroup}> <div style={styles.pinyinLabel}>标准</div> <PinyinVisualizer analysis={result.correct} isCorrect={true} /> </div> <div style={styles.comparisonPinyinGroup}> <div style={styles.pinyinLabel}>你的发音</div> <PinyinVisualizer analysis={result.user} isCorrect={result.pinyinMatch} /> </div> </div> ))} </div> ) ) : ( <div style={styles.noRecognitionResult}> <p>请点击下方的播放按钮，</p> <p>听录音并与标准发音对比。</p> </div> )} </div> <div style={styles.audioComparisonSection}> <button style={styles.audioPlayerButton} onClick={playCorrectTTS}><FaPlayCircle size={18} /> 标准发音</button> {userAudioURL && <button style={styles.audioPlayerButton} onClick={playUserAudio}><FaPlayCircle size={18} /> 你的录音</button>} </div> <div style={styles.comparisonActions}> {(analysis && analysis.isCorrect) ? (<button style={{...styles.actionButton, ...styles.continueButton}} onClick={onContinue}>继续下一个 <FaArrowRight /></button>) : (<button style={{...styles.actionButton, ...styles.retryButton}} onClick={onClose}>再试一次</button>) } </div> </div> </div> ); };
...
const WordCard = ({ words = [], isOpen, onClose, progressKey = 'default' }) => {
  ...
  const [currentIndex, setCurrentIndex] = useState(() => { if (typeof window !== 'undefined') { try { const savedIndex = localStorage.getItem(storageKey); const index = savedIndex ? parseInt(savedIndex, 10) : 0; return index < cards.length ? index : 0; } catch (error) { console.error("读取进度失败", error); return 0; } } return 0; });
  ...
  const audioStreamRef = useRef(null); // ✅ Ref for the original MediaStream
  ...
  // ✅ [最终方案] 使用 Web Audio API 旁路录音，稳定可靠
  const handleListen = useCallback(async (e) => {
      ...
      if (isListening) {
          recognitionRef.current?.stop(); // onend will handle cleanup
          return;
      }
      ...
      try {
          ...
          recognition.onerror = (event) => {
              ...
              // onend will be called automatically to cleanup
          };
          
          // 2. 使用 Web Audio API 进行旁路录音
          ...
          source.connect(destination); // 将原始流导入虚拟输出

          const recorder = new MediaRecorder(destination.stream); // 从虚拟输出录音
