// components/WordCard.js (混合降级策略 + 预加载版)
...
async function toggleFavorite(word) { 
    ...
    if (existing) { store.delete(word.id); return false; } 
        ...
        store.put({ id: word.id, chinese: word.chinese, burmese: word.burmese, mnemonic: word.mnemonic, example: word.example }); 
        return true; 
    } 
}
...
// 全局变量控制播放器
let _currentAudio = null; // 原生 Audio 对象
let _currentAudioUrl = null;

// --- 辅助函数：浏览器原生 TTS (兜底方案) ---
const speakNative = (text, langCode) => {
    return new Promise((resolve) => {
        if (!window.speechSynthesis) return resolve();
        window.speechSynthesis.cancel(); // 停止之前的
        const utterance = new SpeechSynthesisUtterance(text);
        // 简单映射：中文用 zh-CN，缅文尝试用 my-MM (如果系统支持)
        utterance.lang = langCode.startsWith('zh') ? 'zh-CN' : 'my-MM';
        utterance.rate = 1.0;
...
const fetchAudioBlob = async (text, voice, rate) => {
    ...
    const timeoutId = setTimeout(() => controller.abort(), 8000); // 8秒下载超时
...
const playTTS = (text, voice, rate, e) => {
    ...
    return new Promise(async (resolve) => {
        ...
        window.speechSynthesis.cancel(); // 同时清理原生 TTS

        // 2. 设定超时降级机制
        let hasResolved = false;
        const safeResolve = () => { if (!hasResolved) { hasResolved = true; resolve(); } };
        
        // 如果 2 秒内没加载出 API 音频，直接降级到原生 TTS (仅限中文，缅文原生支持差)
        const fallbackTimer = setTimeout(() => {
            if (!hasResolved && voice.startsWith('zh')) {
                console.log("API 超时，降级为原生语音");
                if (_currentAudio) { _currentAudio.pause(); _currentAudio = null; } // 停止正在缓冲的网络音频
                speakNative(text, voice).then(safeResolve);
            }
        }, 2000); // 2秒等待时间
        ...
        try {
            ...
            if (!blob) {
                ...
                if (voice.startsWith('zh')) await speakNative(text, voice);
                safeResolve();
                return;
            }

            // 5. 播放 Blob (使用原生 Audio，兼容性更好)
            ...
            // 绑定事件
            audio.onended = () => { clearTimeout(fallbackTimer); safeResolve(); };
            audio.onerror = () => { clearTimeout(fallbackTimer); safeResolve(); };
            
            // 尝试播放
            await audio.play();
            // 如果 play 成功开始，取消降级倒计时（声音已经出来了）
            // 注意：这里不取消 timer，为了防止虽然 play() 了但一直在 loading 转圈
            // 只有 oncanplaythrough 或者 ontimeupdate 才能证明真的出声了，这里简化处理，保留 timer 做最后保障
...
const PronunciationModal = ({ correctWord, settings, onClose }) => {
    ...
    const startRecording = async () => {
        ...
        try {
            ...
            recognition.onresult = (e) => { if (e.results.length > 0) setUserText(e.results[0][0].transcript.replace(/[.,。，]/g, '')); };
            ...
            setTimeout(() => { if (recognitionRef.current) stopRecording(); }, 5000);
        } catch (err) { console.error("录音启动失败", err); alert("无法访问麦克风"); }
    };
    const stopRecording = () => {
    ...
    useEffect(() => {
        if (recordingState === 'analyzing') {
            const checkTimeout = setTimeout(() => { if (userText) analyzeResult(); else { alert("未检测到清晰语音，请再试一次"); setRecordingState('idle'); } }, 1000);
    ...
    return (
        <div style={modalStyles.overlay}>
            <div style={modalStyles.card}>
                ...
                {recordingState === 'recording' && (<div style={modalStyles.waveContainer}><div style={modalStyles.wave}></div><div style={modalStyles.wave}></div><div style={modalStyles.wave}></div><p style={{color: '#ef4444', fontWeight: 'bold'}}>正在录音...</p></div>)}
                {recordingState === 'analyzing' && (<div style={{margin: '20px 0', color: '#666'}}>正在分析...</div>)}
                {recordingState === 'result' && analysis && (
                    <div style={modalStyles.resultContainer}>
                        <div style={modalStyles.scoreCircle(analysis.score)}><span style={{fontSize: '2.5rem', fontWeight: 'bold'}}>{analysis.score}</span><span style={{fontSize: '0.8rem'}}>分</span></div>
                        <div style={modalStyles.detailRow}>{analysis.details.map((item, i) => (<div key={i} style={modalStyles.charBlock}><div style={{color: item.isMatch ? '#10b981' : '#ef4444', fontSize: '0.9rem'}}>{item.pinyin}</div><div style={{fontSize: '1.5rem', fontWeight: 'bold'}}>{item.char}</div></div>))}</div>
...
const WordCard = ({ words = [], isOpen, onClose, onFinishLesson, hasMore, progressKey = 'default' }) => {
  ...
  // ✅ 新增：预加载下一张卡片的音频 (Pro 优化)
  useEffect(() => {
      if (!activeCards || activeCards.length === 0) return;
      // 预加载下两张卡片
      const nextIndex = (currentIndex + 1) % activeCards.length;
      const nextNextIndex = (currentIndex + 2) % activeCards.length;
      const cardsToPreload = [activeCards[nextIndex], activeCards[nextNextIndex]];

      cardsToPreload.forEach(card => {
          if (!card) return;
          if (card.chinese) fetchAudioBlob(card.chinese, settings.voiceChinese, settings.speechRateChinese);
          if (card.burmese) fetchAudioBlob(card.burmese, settings.voiceBurmese, settings.speechRateBurmese);
      });
  ...
  useEffect(() => {
    ...
    const runAutoPlaySequence = async () => {
        await new Promise(r => setTimeout(r, 500)); // 短暂延迟等待 UI 动画
    ...
    return () => {
        isCancelled = true;
        clearTimeout(autoBrowseTimerRef.current);
        if (_currentAudio) { _currentAudio.pause(); } // 停止网络音频
        window.speechSynthesis.cancel(); // 停止原生音频
    };
