// components/WordCard.js (UI重构版：全新语音对比界面 + 双轨录音修复)
...
import { 
    ...
    FaHeart, FaRegHeart, FaPlay, FaStop, FaVolumeUp, 
    FaTrashAlt, FaRedo, FaCheckCircle, FaTimesCircle
} from 'react-icons/fa';
...
const playTTS = async (text, voice, rate, onEndCallback, e) => { 
    ...
    const playBlob = (blob) => {
        ...
        _howlInstance = new Howl({ src: [audioUrl], format: ['mp3', 'webm'], html5: true, onend: () => { if (onEndCallback) onEndCallback(); }, onloaderror: () => { if (onEndCallback) onEndCallback(); }, onplayerror: () => { if (onEndCallback) onEndCallback(); } }); 
        _howlInstance.play(); 
    };
    try { 
        const cachedBlob = await getTTSFromCache(cacheKey);
        if (cachedBlob) { playBlob(cachedBlob); return; }
        if (typeof navigator !== 'undefined' && !navigator.onLine) { alert("网络已断开"); if (onEndCallback) onEndCallback(); return; }
...
const PronunciationModal = ({ correctWord, settings, onClose }) => {
    ...
    const startRecording = async () => {
        try {
            ...
            mediaRecorder.onstop = () => {
                ...
                setAudioUrl(url);
                stream.getTracks().forEach(track => track.stop()); // 关麦
            };
    ...
    // 只有当文字和音频都准备好时，才进行分析
    useEffect(() => {
        if (recordingState === 'analyzing' && userText) {
            analyzeResult();
        } else if (recordingState === 'analyzing' && !userText) {
             // 没识别到文字
             setTimeout(() => {
                 if (!userText) {
                     alert("未检测到语音，请大声一点");
    ...
    const analyzeResult = () => {
        ...
        const details = correctPinyin.map((cpy, i) => {
            ...
            const isMatch = cParts.rawPinyin === uParts.rawPinyin; // 全匹配
        ...
        // 播放音效
        if (score === 100) playSoundEffect('correct');
        else playSoundEffect('incorrect');
    };
    ...
    const playUserAudio = () => {
        if (audioUrl) {
            const sound = new Howl({ src: [audioUrl], format: ['webm'], html5: true });
            sound.play();
        }
    };
        ...
        <div style={modalStyles.overlay}>
            <div style={modalStyles.card}>
                ...
                {recordingState !== 'result' && (
                    <div style={modalStyles.bigWord}>{correctWord}</div>
                )}

                {/* 状态：录音中 (波形动画模拟) */}
                {recordingState === 'recording' && (
                    <div style={modalStyles.waveContainer}>
                        <div style={modalStyles.wave}></div>
                        <div style={modalStyles.wave}></div>
                        <div style={modalStyles.wave}></div>
                        ...
                        <div style={modalStyles.detailRow}>
                            {analysis.details.map((item, i) => (
                                <div key={i} style={modalStyles.charBlock}>
                                    ...
                                    {!item.isMatch && <div style={{color: '#ef4444', fontSize: '0.8rem'}}>{item.uPinyin || '?'}</div>}
                                </div>
                            ))}
                        </div>
                        <div style={modalStyles.audioControls}>
                            <button style={modalStyles.playBtn} onClick={() => playTTS(correctWord, settings.voiceChinese, settings.speechRateChinese)}>
                                <FaPlay size={12} /> 标准音
                            </button>
                            <button style={modalStyles.playBtn} onClick={playUserAudio}>
                                <FaPlay size={12} /> 我的录音
                            </button>
                        </div>
                    </div>
                ...
                <div style={modalStyles.footer}>
                    {recordingState === 'idle' && (
                        <button style={modalStyles.recordBtn} onClick={startRecording}>
                            <FaMicrophone size={24} />
                        </button>
                    )}
                    
                    {recordingState === 'recording' && (
                        <button style={{...modalStyles.recordBtn, background: '#ef4444'}} onClick={stopRecording}>
                            <FaStop size={24} />
                        </button>
                    )}

                    {recordingState === 'result' && (
                        <button style={modalStyles.retryBtn} onClick={reset}>
                            <FaRedo /> 再试一次
                        </button>
                    )}
                </div>
...
const WordCard = ({ words = [], isOpen, onClose, onFinishLesson, hasMore, progressKey = 'default' }) => {
  ...
  const [isListening, setIsListening] = useState(false); // 用于快速语音识别（无回放）
  const [isComparisonOpen, setIsComparisonOpen] = useState(false); // 用于打开新的评测弹窗
  ...
  // 快速语音识别逻辑 (不需要回放，只转文字，不改动)
  const handleQuickListen = useCallback((e) => {
    e.stopPropagation();
    ...
    try {
        ...
        recognition.onresult = (event) => { 
            ...
            // 快速识别后直接弹窗（带文字） - 可选，这里只做演示，保持之前的逻辑
      ...
      <animated.div style={{ ...styles.fullScreen, ...backgroundStyle, ...style }}>
        ...
        {activeCards.length > 0 && currentCard ? (
            ...
            <div style={styles.completionContainer}>
                ...
                <p>你已学完本组所有单词。</p>
                {hasMore ? (
                    <button style={{...styles.knowButton, ...styles.knowButtonBase, marginTop: '20px', background: 'linear-gradient(135deg, #3b82f6, #2563eb)'}} onClick={() => { if (onFinishLesson) onFinishLesson(); }}>进入下一课 <FaArrowRight style={{marginLeft: '8px'}}/></button>
                ) : (
                    <p style={{marginTop: '10px', fontSize: '0.9rem', opacity: 0.8}}>太棒了！该等级所有单词已学完！</p>
                )}
                <button style={{...styles.knowButtonBase, background: 'transparent', border: '1px solid rgba(255,255,255,0.3)', marginTop: '15px'}} onClick={onClose}>关闭</button>
            </div>
        )}
        ...
        {currentCard && (
            <div style={styles.rightControls} data-no-gesture="true">
                ...
                {/* 快速听写按钮 (简单模式) */}
                <button style={styles.rightIconButton} onClick={handleQuickListen} title="快速语音识别">
