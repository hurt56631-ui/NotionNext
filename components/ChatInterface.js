// /components/ChatInterface.js (最终解决方案 - 采用Flexbox布局)
...
export default function ChatInterface({ chatId, currentUser, peerUser }) {
  ...
  // --- 移除 ---
  // const [footerHeight, setFooterHeight] = useState(0); // 不再需要
  // const [keyboardOffset, setKeyboardOffset] = useState(0); // 不再需要
  ...
  // --- 移除 ---
  // useEffect(() => { ... }); // 测量 footer 高度的 useEffect 已被移除
  ...
  // ... 省略 handleRecallMessage, handleDeleteMessage, sendCorrection, getMyInputPrompt 等未改变的函数 ...
  ...
  return (
    ...
    <div className="flex flex-col w-full bg-white text-black" style={{ height: '100dvh' }}>
      ...
      {/* 内容区: flex-grow 自动填充剩余空间, overflow-y-auto 内部滚动 */}
      <main className="flex-grow overflow-y-auto w-full thin-scrollbar relative">
         <Virtuoso 
            ref={virtuosoRef} 
            style={{ height: '100%' }} // Virtuoso 需要100%高度来填充父容器
