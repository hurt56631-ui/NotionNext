...
export default function ChatInterface({ chatId, currentUser, peerUser }) {
  ...
  return (
    // ✅ 核心修复：在外层容器上添加 `overflow-hidden`
    // 这会强制容器本身不可滚动，从而确保内部的 flex 布局 (固定头、固定尾、中间滚动) 能正确生效
    <div className="flex flex-col w-full bg-white text-black overflow-hidden" style={{ height: '100dvh' }}>
          ...
          <AnimatePresence>
            {unreadCount > 0 && (
              <motion.button 
                ...
                style={{ bottom: `calc(70px + ${keyboardHeight}px + 1rem)`}}
              >
