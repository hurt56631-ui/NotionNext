...
const parseSingleTranslation = (text) => { const translationMatch = text.match(/翻译:：\s*(.*?)\s*回译:/is); const backTranslationMatch = text.match(/回译:：\s*(.*)/is); if (translationMatch && backTranslationMatch) { return { translation: translationMatch[1].trim(), backTranslation: backTranslationMatch[1].trim() }; } const firstLine = text.split(/\r?\n/).find(l => l.trim().length > 0) || text; return { translation: firstLine.trim(), backTranslation: "解析失败" }; };
...
export default function ChatInterface({ chatId, currentUser, peerUser }) {
    ...
    const handleRecallMessage = async (message) => { if (message.uid !== user.uid) return; const messageRef = doc(db, `privateChats/${chatId}/messages`, message.id); try { await updateDoc(messageRef, { recalled: true }); } catch (error) { console.error("撤回消息失败:", error); alert("撤回失败"); } };
    const handleDeleteMessage = async (message) => { if (message.uid !== user.uid) return; if (!window.confirm("确定要删除这条消息吗？")) return; const messageRef = doc(db, `privateChats/${chatId}/messages`, message.id); try { await deleteDoc(messageRef); } catch (error) { console.error("删除消息失败:", error); alert("删除失败"); } };
    const sendCorrection = async () => { if (!correctionMode.active || !correctionMode.message || !correctionMode.text.trim()) return; const messageRef = doc(db, `privateChats/${chatId}/messages`, correctionMode.message.id); try { await updateDoc(messageRef, { correction: { originalText: correctionMode.message.text, correctedText: correctionMode.text.trim() } }); setCorrectionMode({ active: false, message: null, text: '' }); } catch (error) { console.error("发送更正失败:", error); alert("更正失败"); } };
    const getMyInputPrompt = (sourceLang, targetLang) => `你是一位精通${sourceLang}和${targetLang}的双语翻译专家。请将以下${sourceLang}文本翻译成${targetLang}，并提供一个从${targetLang}回译到${sourceLang}的简洁版本，以验证翻译质量。请严格按照以下格式返回，不要添加任何额外解释：\n翻译：这里是翻译成${targetLang}的内容\n回译：这里是回译成${sourceLang}的内容`;
