// /components/ChatInterface.js (V18 - Backgrounds & Keyboard Handling)
...
export default function ChatInterface({ chatId, currentUser, peerUser }) {
  ...
  const [cfg, setCfg] = useState(() => {
      ...
      try {
          const savedCfg = localStorage.getItem("private_chat_settings_v3");
          return savedCfg ? { ...defaultSettings, ...JSON.parse(savedCfg) } : defaultSettings;
      } catch {
  ...
  useEffect(() => {
      if (typeof window !== 'undefined') {
          localStorage.setItem("private_chat_settings_v3", JSON.stringify(cfg));
      }
  }, [cfg]);
  ...
  useEffect(() => {
      try {
          ...
          if (saved) setBackgroundDataUrl(saved);
      } catch (e) {
  ...
  useEffect(() => {
      ...
      const onViewportChange = () => {
        ...
        if (vv) {
          // Modern API for reliable keyboard height detection
          const keyboardHeight = Math.max(0, window.innerHeight - vv.height - vv.offsetTop);
          setKeyboardOffset(keyboardHeight);
        } else {
          // Fallback for older browsers
          const keyboardHeight = Math.max(0, initialInnerHeight - window.innerHeight);
          setKeyboardOffset(keyboardHeight);
        }
        // Also update footer height as its layout might change
...
回译：这里是回译成${sourceLang}的内容`;
  ...
  const handleDeleteAllMessages = async () => {
      if (!window.confirm(`确定要删除与 ${peerUser?.displayName} 的全部聊天记录吗？此操作不可恢复！`)) return;
      alert("删除全部记录功能待实现。");
  };
  const handleBlockUser = async () => {
      if (!window.confirm(`确定要拉黑 ${peerUser?.displayName} 吗？`)) return;
      alert("拉黑功能待实现。");
  };
  ...
  return (
    <div className="flex flex-col w-full h-full bg-white text-black" style={{ height: '100dvh' }}>
        ...
        <main 
            ...
            style={{ 
                bottom: `${footerHeight + keyboardOffset}px`, 
                backgroundImage: backgroundDataUrl ? `linear-gradient(rgba(0,0,0,0.15), rgba(0,0,0,0.15)), url(${backgroundDataUrl})` : undefined,
